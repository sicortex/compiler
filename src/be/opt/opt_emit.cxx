...
Valid_doloop_condition( BB_NODE *bb_end, const OPT_STAB *opt_stab )
{
  ...
  if ( cond_br->Op() != OPC_TRUEBR && cond_br->Op() != OPC_FALSEBR ) {
    DevWarn( "Valid_doloop_condition(BB:%" PRIuPTR ") not true/falsebr",
	    bb_end->Id() );
    return FALSE;
  }
  ...
  if ( rhs->Kind() != CK_OP || ! OPERATOR_is_compare(rhs->Opr()) ) {
    DevWarn( "Valid_doloop_condition(BB:%" PRIuPTR ") cond_br not compare",
	    bb_end->Id() );
    return FALSE;
  }
  ...
  if ( index_cr == NULL ) {
    DevWarn( "Valid_doloop_condition(BB:%" PRIuPTR ") no index in phis",
	    bb_end->Id() );
    return FALSE;
  }
  ...
  if (found_one_side_use_index == FALSE) {
    DevWarn( "Valid_doloop_condition(BB:%" PRIuPTR ") can't find index var on comparison",
	    bb_end->Id() );
    return FALSE;
  }
  ...
  FOR_ALL_NODE( sr, stmt_iter, Init() ) {
    if (sr->Op() != OPC_LABEL && sr != bb_end->Last_stmtrep()) {
      DevWarn( "Valid_doloop_condition(BB:%" PRIuPTR ") more than one stmt",
       	      bb_end->Id() );
      return FALSE;
    }
  }
	      ...
	      ALIAS_MANAGER *alias_mgr )
{
  ...
  
  {
	...
	if (Can_raise_to_doloop(bb->Innermost(), TRUE/*repair*/, Htable())) {
	  if (_trace)
	    fprintf(TFile, "EMIT: BB%" PRIuPTR " can raise to do-loop\n", bb->Id());
	} else {
	  if (_trace)
	    fprintf(TFile, "EMIT: BB%" PRIuPTR " cannot raise to do-loop\n", bb->Id());
	}
...
#if defined( KEY) // bug 8327: the incr stmt has been optimized to something else
				...
				WN **first_stmt, WN **last_stmt)
{
  ...
  if (last_bb == NULL) {
   ...
   DevWarn("fixing region %d with first_bb BB%" PRIuPTR " to have region_end BB%" PRIuPTR "\n",
