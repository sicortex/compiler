...
static INT Inner_Loops_Alowed()
{
  if(LNO_Full_Unroll_Aggressive)
    return MAX_INNER_LOOPS;
  else return 3; //original
}
#endif
...
#ifdef KEY
           Num_Inner_Loops(wn) <= Inner_Loops_Alowed()) {
          //bug 10644
          if(Num_Inner_Loops(wn) == Inner_Loops_Alowed() &&
...
//    fprintf(stdout, "LNO processing procedure: %s\n", Cur_PU_Name);
...

  {
...
#ifdef KEY 
    if (LNO_Build_Scalar_Reductions || Enable_Cfold_Reassociate){
#else
	...
	if(red_manager){
	  red_manager->Build(func_nd, TRUE, FALSE);//scalar
          if (Enable_Cfold_Reassociate)//array only if roundoff>=2
            red_manager->Build(func_nd,FALSE,TRUE,Array_Dependence_Graph);
        }
#endif
       }          
  ...
  if(LNO_Indexed_Load){
   ...
   Build_CG_Dependence_Graph(func_nd);
    ...
    if (LNO_Cse && Enable_Cfold_Reassociate) { 
...
void DO_LOOP_INFO::Print(FILE *fp, INT indentation)
{
  ...
  if (Step) {
    ...
    fprintf(fp, "<null>\n");
  fprintf(fp,"%sWe estimate this loop has %" SCNd64 " iterations.  %s\n",
	  buf, Est_Num_Iterations, Num_Iterations_Symbolic?"<symbolic>":"");
  if (Est_Max_Iterations_Index != -1) 
    fprintf(fp,"%sThis loop has at most %" SCNd64 " iterations due to array bounds.\n",
...
//  Hoist_Outer_Invar(func_nd);
  ...
  if (LNO_Run_Simd && LNO_Run_Simd_Set && !Skip_Simd
#ifdef TARG_X8664
      && Is_Target_SSE2()
#endif
     )
...
if(red_manager){
  red_manager->Erase(func_nd);
  red_manager->Build(func_nd, TRUE, FALSE);//scalar
  if (Enable_Cfold_Reassociate) //array
   red_manager->Build(func_nd,FALSE,TRUE,Array_Dependence_Graph);
 }
#endif
#endif
