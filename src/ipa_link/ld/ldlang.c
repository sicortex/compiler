...
#ifdef KEY
// IPA archive contains both real and IPA objects.  Print out the two object
// types to help user debugging.
static void
handle_ipa_archive_mismatch (lang_input_statement_type *entry)
{
  bfd *member;

  einfo(_("%B: cannot mix regular and IPA objects in same archive\n"),
        entry->the_bfd);

  // List all the non-IPA objects in the archive.
  fprintf(stderr, "regular objects: ");
  member = bfd_openr_next_archived_file (entry->the_bfd, NULL);
  while (member != NULL) {
    ...
    // it's an object
    if (!ipa_is_whirl(member)) {	// non-IPA object
      einfo (_(" %B"), member);
    }
    member = bfd_openr_next_archived_file(entry->the_bfd, member);
  }

  // List all the IPA objects in the archive.
  fprintf(stderr, "\nIPA objects:     ");
  member = bfd_openr_next_archived_file (entry->the_bfd, NULL);
  while (member != NULL) {
    if (!bfd_check_format (member, bfd_object)) {
    ...
    // it's an object
    if (ipa_is_whirl(member)) {	// IPA object
      einfo (_(" %B"), member);
    }
    member = bfd_openr_next_archived_file(entry->the_bfd, member);
  }
