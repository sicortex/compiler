...
BOOL PF_LG::Is_Outer_Tile () {
  ...
  for (i=ref_depth; i!=_depth; i--) {
    dli = ln->Get_LoopInfo ();
//    printf ("(is_outer_tile: depth %d e_n_i %" SCNd64 ")\n",
...
//    arrays in a loop.
//---------------------------------------------------------------------------
if(LNO_Run_Stream_Prefetch && //flag turned on
...
void PF_LG::Print (FILE *fp) {
  ...
  INT i;
  for (i=0; i<Get_Dim(); i++)
    fprintf (fp, "          %4" SCNd64 " %3" SCNd64 " %3" SCNd64 "\n", 
	_c[i], _min_iter[i], _max_iter[i]);
  fprintf (fp, "          Distance (bytes): Min %" SCNd64 ", Max %" SCNd64 "\n",
           _min_dist, _max_dist);
