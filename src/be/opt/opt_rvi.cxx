...
RVI::Get_bb_local_attributes( BB_NODE *bb )
{
  ...
  if ( first == NULL ) {
	...
	if ( Tracing() ) {
	  fprintf( TFile, 
	    "Get_bb_local_attributes: split BB:%" PRIdPTR " into BB:%" PRIdPTR "/BB:%" PRIdPTR "\n",
...
#ifdef Is_True_On
  if ( Tracing() ) {
    fprintf( TFile, "BB:%" PRIdPTR " Loc_appear: ", bb->Id() );
    bb->Loc_appear()->Print(TFile);
    fprintf( TFile, "\n" );
    fprintf( TFile, "BB:%" PRIdPTR " Loc_def: ", bb->Id() );
    bb->Loc_def()->Print(TFile);
    fprintf( TFile, "\n" );
    fprintf( TFile, "BB:%" PRIdPTR " Loc_upwd: ", bb->Id() );
    bb->Loc_upwd()->Print(TFile);
    fprintf( TFile, "\n" );
    fprintf( TFile, "BB:%" PRIdPTR " Unstored_defs: ", bb->Id() );
    bb->Unstored_defs()->Print(TFile);
    fprintf( TFile, "\n" );
    fprintf( TFile, "BB:%" PRIdPTR " Loc_mu_ref: ", bb->Id() );
    if ( bb->Loc_mu_ref() )
      bb->Loc_mu_ref()->Print(TFile);
    else
      fprintf( TFile, "NULL" );
    fprintf( TFile, "\n" );
    fprintf( TFile, "BB:%" PRIdPTR " Loc_chi_def: ", bb->Id() );
    if ( bb->Loc_chi_def() )
    ...
    if ( bb->Last_stid_bitpos() != ILLEGAL_BP ) {
      fprintf( TFile, "BB:%" PRIdPTR " Last_stid_bitpos: %d %s\n", 
...
RVI::Get_backward_dataflow( void )
{
  ...
  if ( Tracing() ) {
    for ( INT bbi = 0; bbi < Dfs_vec_size(); bbi++ ) {
      BB_NODE *bb = Dfs_vec(bbi);
      fprintf( TFile, "<RVI::Get_backward_dataflow: bb:%" PRIdPTR " init l-a-e:   ",
		bb->Id() );
      bb->Live_at_exit()->Print( TFile );
      fprintf( TFile, "\n" );
      fprintf( TFile, "<RVI::Get_backward_dataflow: bb:%" PRIdPTR " init live-out:",
		bb->Id() );
...
RVI::Get_dataflow_equations( void )
{
  ...
  if ( Tracing() ) {
    for ( INT bbi = 0; bbi < Dfs_vec_size(); bbi++ ) {
      ...
      fprintf ( TFile, "BB:%" PRIdPTR " Defreach: ", bb->Id() );
      bb->Defreach()->Print(TFile);
      fprintf ( TFile, "\n" );
      fprintf ( TFile, "BB:%" PRIdPTR " Live_at_exit: ", bb->Id() );
      bb->Live_at_exit()->Print(TFile);
      fprintf ( TFile, "\n" );
      fprintf ( TFile, "BB:%" PRIdPTR " Live_out: ", bb->Id() );
      bb->Live_out()->Print(TFile);
      fprintf ( TFile, "\n" );
      fprintf ( TFile, "BB:%" PRIdPTR " Unstored_defs: ", bb->Id() );
      bb->Unstored_defs()->Print(TFile);
  ...
  for ( bbi = 0; bbi < Dfs_vec_size(); bbi++ ) {
    ...
    if ( Tracing() ) {
      fprintf ( TFile, "BB:%" PRIdPTR " Live-range (defreach): ", bb->Id() );
      bb->Defreach()->Print(TFile);
      fprintf ( TFile, "\n" );
    }
  }
...
RVI::Get_backward_lda_dataflow( void )
{
  ...
  if ( Tracing() ) {
    for ( INT bbi = 0; bbi < Dfs_vec_size(); bbi++ ) {
      BB_NODE *bb = Dfs_vec(bbi);
      fprintf( TFile, "<RVI::Get_backward_dataflow: init l-a-e:bb:%" PRIdPTR ":",
		bb->Id() );
      bb->Live_at_exit()->Print( TFile );
      fprintf( TFile, "\n" );
    }
			    ...
			    CODEREP *cr)
{
  if (opt_stab->Aux_stab_entry(cr->Aux_id())->Itab_bitpos() ==
    ...
    Is_Trace(Tracing(),
	     (TFile, "Setup_bitpos: Aux id %" PRIdPTR " already has bitpos %d\n",
	      cr->Aux_id(),
