...
#ifdef KEY
#define __STDC_LIMIT_MACROS		// Get UINT16_MAX from stdint.h.
#include <stdint.h>
#endif
...
static int fp_count = 0;		// fp OPs
static int memory_count = 0;		// memory OPs
...
// Workaround the mul.d hardware bug.  Bug 13027.

// If forward scheduling, positive if in shadow of mul.d.  If backward
// scheduling, positive if in shadow of an OP that disrupts mul.d.
static int muld_shadow;

// Number of NOPs to add between the mul.d and the OP that disrupts it.
static int add_nops;
#endif
		   ...
		   HBS_TYPE type)
{
  ...
  if (Trace_HB) {
    ...
    fprintf (TFile, "old: %" SCNd64 ", new: %" SCNd64 "\n", TN_value(old_ofst_tn), 
...
// ======================================================================
// OP is to be scheduled next.  If scheduling it violates any REGANTI
// dependence, create a copy to rename the register and return the copy.
// ======================================================================
OP *
HB_Schedule::Fixup_Reganti (OP *op, BOOL is_fwd)
{
  int i;

  if (!HBS_Relax_Reganti())
    return NULL;

  Is_True(!is_fwd, ("Fixup_Reganti: NYI for forward scheduling"));
  ...
  // See if any REGANTI dependence is violated.
  for (arcs = OP_preds(op); arcs != NULL; arcs = ARC_LIST_rest(arcs)) {
    ARC *arc = ARC_LIST_first(arcs);
    OP *pred_op = ARC_pred(arc);
    OPSCH *pred_opsch = OP_opsch(pred_op, _hb_map);
	...
	// These TNs should have been filtered out.
	Is_True(!TN_is_dedicated(old_tn) && TN_is_global_reg(old_tn),
		("Fixup_Reganti: unexpected TN"));
	new_tn = Dup_TN(old_tn);
	break;
      }
    }
  }

  if (new_tn == NULL)	// Quit if no copy needed.
    return NULL;

  // Insert copy.
  OPS ops = OPS_EMPTY;
  Exp_COPY(new_tn, old_tn, &ops);
  OP *copy_op = OPS_last(&ops);
  OP_srcpos(copy_op) = OP_srcpos(op);
  BB_Insert_Ops_Before(OP_bb(op), op, &ops);

  // The TNs have been renamed.  Must emit the new schedule.
  Set_hbs_must_emit_sched();
  ...
  CG_DEP_New_Noop(copy_op);

  // Fake a scycle for copy_op.  A rough scycle is ok because only the first
  // scheduling pass inserts these copies.  The final schedule is determined by
  // the second scheduling pass.
  OPSCH *op_opsch = OP_opsch(op, _hb_map);
  OPSCH_scycle(copy_opsch) = OPSCH_scycle(op_opsch);

  // For all the REGANTI preds that are scheduled, rename the TN to the new TN.
  for (arcs = OP_preds(op); arcs != NULL; arcs = ARC_LIST_rest(arcs)) {
    ARC *arc = ARC_LIST_first(arcs);
    if (ARC_kind(arc) == CG_DEP_REGANTI) {
      OP *pred_op = ARC_pred(arc);
      OPSCH *pred_opsch = OP_opsch(pred_op, _hb_map);
      if (OPSCH_scheduled(pred_opsch)) {
	Set_OP_opnd(pred_op, ARC_opnd(arc), new_tn);
      }
    }
  }
...
// Return TRUE if all the arcs between PRED and SUCC can be ignored for the
// purpose of scheduling.
BOOL
HB_Schedule::Ignore_Arcs (OPSCH *pred_opsch, OPSCH *succ_opsch, ARC_LIST *arcs)
{
  OP *pred_op = OPSCH_op(pred_opsch);
  OP *succ_op = OPSCH_op(succ_opsch);

  if (HBS_Relax_Reganti()) {
    for ( ; arcs != NULL; arcs = ARC_LIST_rest(arcs)) {
      ARC *arc = ARC_LIST_first(arcs);
      if (ARC_pred(arc) == pred_op &&
	  ARC_succ(arc) == succ_op) {
	if (ARC_kind(arc) != CG_DEP_REGANTI)	// Ignore REGANTI only.
	  return FALSE;
	TN *tn = OP_opnd(pred_op, ARC_opnd(arc));
	// We handle REGANTI of non-dedicated TN only.
	...
	if (!TN_is_global_reg(tn))	// todo: revisit this restriction
	  return FALSE;
	Is_True(TN_is_global_reg(tn),
		("Ignore_Arcs: unexpected REGANTI for local TN"));
      }
    }
    return TRUE;
  }
...
// Return TRUE if all the unscheduled succs of OP can be ignored, so that OP
// can be made ready despite having unscheduled succs.  This assumes backward
// scheduling.
BOOL
HB_Schedule::Ignore_Unsched_Succs (OPSCH *opsch)
{
  OP *op = OPSCH_op(opsch);
  ARC_LIST *arcs;
  ...
  for (arcs = OP_succs(op); arcs != NULL; arcs = ARC_LIST_rest(arcs)) {
	...
	!Ignore_Arcs(opsch, succ_opsch, OP_succs(op))) {
      return FALSE;
    }
#ifdef TARG_X8664
    // If OP reads rflags and SUCC_OP modifies rflags, don't swap them.
    if (OP_reads_rflags(op) &&
	OP_change_rflags(succ_op)) {
      return FALSE;
    }
#endif
  }
...
Init_OPSCH_For_BB (BB *bb, BB_MAP value_map, BOOL compute_bitsets,
		   MEM_POOL *pool, BB_OP_MAP priorities = NULL)
{
  OP *op;
  ARC_LIST *arcs;
  ARC *arc;
  ...
  FOR_ALL_BB_OPs_FWD (bb, op) {
    ...
    if (priorities != NULL)
      OPSCH_priority(opsch) = BB_OP_MAP32_Get(priorities, op);
    if (compute_bitsets) {
    ...
    if (OP_prefetch(op))	// Count number of prefetches.
      unsched_prefetch_count++;
    if (OP_flop(op))		// Count number of FP OPs.
      fp_count++;
    if (OP_memory(op))		// Count number of memory OPs.
      memory_count++;
#endif
  }
...
#ifdef KEY
// ======================================================================
// Add an arc between every OP to force the scheduler to preserve the
// existing order.
// ======================================================================
static void
Add_No_Reorder_Arcs (BB *bb)
{
  OP *op;
  ...
  FOR_ALL_BB_OPs_FWD(bb, op) {
    ARC_LIST *arcs;
    if (OP_next(op) != NULL) {
      new_arc_with_latency(CG_DEP_MISC, op, OP_next(op), 0, 0, 0, FALSE);
    }
  }
}
...
#ifdef KEY
  OP *op = OP_VECTOR_element(sched->ready_vector(), 0);
  BB *bb = OP_bb(op);
  FOR_ALL_BB_OPs_FWD(bb, op) {
    if (OP_preds(op) == NULL)
...
Compute_OPSCH (BB *bb, BB_MAP value_map, MEM_POOL *pool, BOOL compute_bitsets,
	       BOOL is_fwd, BB_OP_MAP priorities)
{
  INT max_lstart = 0;

  Init_OPSCH_For_BB (bb, value_map, compute_bitsets, pool, priorities);
...
#ifdef TARG_MIPS
  // Prefetches must be spaced far apart and be near to the top of the BB.
  // Help backward scheduling to determine when it is time to schedule a
  // prefetch.
  if (Is_Target_Mips5kf()) {
    OP *op = OP_VECTOR_element(_ready_vector, 0);	// pick any OP in BB
    if (BB_keep_prefetch(OP_bb(op))) {
      // Estimate the number of cycles needed by the remaining instructions.
      float ops_per_cycle = 1.4;		// Estimate issue rate.
      float cycles_remaining = _unsched_count / ops_per_cycle;
      int prefetch_latency = 16;
      float cycles_needed = (unsched_prefetch_count - 1 ) * prefetch_latency;
      _sched_prefetch_first = (cycles_needed > cycles_remaining) ? TRUE : FALSE;
    }
  }
#endif
...
// ======================================================================
// Add OP to ready vector.
// ======================================================================
void
HB_Schedule::Add_OP_To_Ready_Vector (OPSCH *opsch)
{
  Is_True(!OPSCH_ready(opsch),
	  ("Add_OP_To_Ready_Vector: OP already in ready vector"));
  VECTOR_Add_Element(_ready_vector, OPSCH_op(opsch));
  Set_OPSCH_ready(opsch);
}
#endif
...
HB_Schedule::Add_OP_To_Sched_Vector (OP *op, BOOL is_fwd)
{
  ...
  OP *copy_op = Fixup_Reganti(op, is_fwd);
	...
	if (!Is_Ldst_Addiu_Pair (pred_opsch, opsch, pred_op, op, arc)) 
	  ...
	  if (!OPSCH_ready(pred_opsch)) {
		...
		Ignore_Unsched_Succs(pred_opsch)) {
	      Add_OP_To_Ready_Vector(pred_opsch);
	    }
	  }
#else
	  if (OPSCH_num_succs(pred_opsch) == 0) {
...
#ifdef ICE9A_HW_WORKAROUND
  while (add_nops > 0) {
    // Fabricate a NOP and insert it anywhere in the BB.
    OPS ops = OPS_EMPTY;
    ...
    CG_DEP_New_Noop(nop);  // Bug 13083

    VECTOR_Add_Element (_sched_vector, nop);
    add_nops--;
  }
#endif
...
#ifdef KEY
  if (copy_op != NULL)
    VECTOR_Add_Element (_sched_vector, copy_op);
#endif
...
#ifdef KEY
  BOOL prefetch_first = _cur_sched->Sched_Prefetch_First();
  BOOL non_prefetch_first = _cur_sched->HBS_Drop_Unsched_Prefetches();

  // Schedule prefetches toward the top of the BB.  Bug 15087.
  if (!prefetch_first &&
      !non_prefetch_first) {
    non_prefetch_first = TRUE;
  }

  Is_True(!(prefetch_first && non_prefetch_first),
      ("Is_OP_Better: cannot have both prefetch_first and non_prefetch_first"));

  if (non_prefetch_first) {	// Schedule non-prefetch first.
	...
	cur_scycle >= best_scycle)
      return TRUE;
  } else if (prefetch_first) {	// Schedule prefetch first.
    if (OP_prefetch(cur_op) && !OP_prefetch(best_op) &&
	cur_scycle >= best_scycle)
      return TRUE;
    if (!OP_prefetch(cur_op) && OP_prefetch(best_op) &&
...
#ifdef KEY
  if (OPSCH_priority(cur_opsch) > OPSCH_priority(best_opsch))
    return TRUE;
  if (OPSCH_priority(cur_opsch) < OPSCH_priority(best_opsch))
    return FALSE;
...
#ifdef TARG_MIPS
    if (!CGTARG_Is_Delay_Slot_Op(cur_op))
      continue;
#endif
...
#ifdef KEY
// ======================================================================
// Determine how to schedule an OP that references a TN whose live-range we
// want to minimize.  Return 1 if OP must be scheduled now, -1 if OP should not
// be scheduled now, or 0 if it is ok but not necessary to schedule OP now.
// ======================================================================
int
Priority_Selector::Sched_OP_With_Precious_TN (OP *op)
{
#ifdef TARG_X8664

#define IS_MULT_DIV(opcode)	\
  ((opcode) == TOP_mul32 ||	\
   (opcode) == TOP_imulx32 ||	\
   (opcode) == TOP_div32 ||	\
   (opcode) == TOP_idiv32)

  // We are scheduling to minimize regs.  Under m32, schedule long-latency
  // mul/div immediately before the copy that copies the result out of rax/rdx,
  // in order to free up rax/rdx for potential allocation to OPs requiring
  // byteable registers.  (We are performing backward scheduling.  If the
  // mul/div is ready, it means the copy has already been scheduled.)
  // Bug 14456.
  if (Is_Target_32bit() &&
      IS_MULT_DIV(OP_code(op))) {
    return 1;
  }
#endif

#if defined(TARG_X8664) || defined(TARG_MIPS)
  // A "precious" TN is one whose live-range we want to minimize in order to
  // avoid spilling, because spilling it is either undesirable or impossible.
  // See if OP references a precious TN.  If so, determine if OP must be
  // scheduled in the current cycle as follows.  Find PRED_OP that defines the
  // precious TN.  Find the OPs that reference the precious TN(s) defined by
  // PRED_OP.  These are OP's "siblings".  Schedule OP and the siblings
  // together as a group so that PRED_OP can be scheduled immediately
  // afterwards, thereby shortening the live ranges of the precious TN.  (This
  // assumes backwards scheduling.)  Handle three cases for OP:
  ...
  for (int opndnum = 0; opndnum < OP_opnds(op); opndnum++) {
...
#endif
#ifdef TARG_MIPS
	// Schedule definition and uses of fcc together.  (SiCortex 10768)
	TN_register_class(op_opnd_tn) == ISA_REGISTER_CLASS_fcc
#endif
	) {
      // OP_OPND_TN is a precious TN.
	...
	if (!OP_Defs_TN(pred_op, op_opnd_tn))
	  continue;
	// PRED_OP defines the precious TN.
...
#endif
#ifdef TARG_MIPS
	      TN_register_class(pred_result_tn) == ISA_REGISTER_CLASS_fcc
#endif
	      ) {
	    // Pred's RESULT_TN is a precious TN.  For each of OP's siblings
	    // that references RESULT_TN, see if it is ready or scheduled.
...
HB_Schedule::Put_Sched_Vector_Into_BB (BB *bb, BBSCH *bbsch, BOOL is_fwd)
{
  ...
  if (cur_cycle < _max_sched) 
  {
    ...
    int count = VECTOR_count(_sched_vector);
    for (i = (is_fwd) ? 0 : count - 1;
	 (is_fwd) ? i < count : i >= 0; 
	 (is_fwd) ? i++ : i--) {
      OP *op = OP_VECTOR_element(_sched_vector, i);
      BB_Append_Op(bb, op);
  ...
  // accurate asm annotation.
    ...
    // Must emit the schedule if the OPs have been changed specifically for the
    // schedule.
    Is_True(!HBS_Must_Emit_Sched(),
	    ("Put_Sched_Vector_Into_BB: must emit sched after changing OPs"));
    for (i = VECTOR_count(_sched_vector) - 1; i >= 0; i--) {
      OP *op = OP_VECTOR_element(_sched_vector, i);
...
#if defined(TARG_X8664) || defined(TARG_MIPS)
    // Minimize certain live-ranges to reduce LRA register pressure.
    //
    // X8664: Schedule the OPs that have preallocated TNs as close as possible
    // to the copies that move the preallocated TNs to/from regular TNs.  This
    // is needed to shorten the live range of the registers corresponding to
    // the preallocated TNs; otherwise LRA can run out of registers and die.
    // Bug 6081.
    //
    // MIPS: Minimize fcc live-range.
    if (Cur_Sched->HBS_Minimize_Regs()) {
      INT status = Sched_OP_With_Precious_TN(cur_op);
...
#endif
#ifdef TARG_MIPS
  // Must pick an OP referencing fcc if that's the only ready OP.
#endif
  if (_best_op == NULL)
...
{
#ifdef ICE9A_HW_WORKAROUND
  // Don't schedule a FP OP near a mul.d, unless it is another mul.d, in order
  // to avoid mul.d hardware bug.  Bug 13027.
  if (CG_muld == 1 &&
      muld_shadow > 0 &&
      ((is_fwd && OP_disrupts_muld(cur_op)) ||
       (!is_fwd && OP_is_like_muld(cur_op)))) {
    return FALSE;
  }
#endif
			     ...
			     BB_OP_MAP priorities)
{
  ...
  OP *op;
  FOR_ALL_BB_OPs_FWD(bb, op) { 
    // Make space for possibly 3 NOPs to workaround mul.d bug.
    if (CG_muld == 1 &&
	OP_is_like_muld(op))
      length += 3;
  }
...
#ifdef ICE9A_HW_WORKAROUND
	if (Can_Schedule_Op(cur_op, cur_time, priority_fn->Is_Fwd_Schedule()))
	  break;
	if (CG_muld == 1 &&
	    muld_shadow > 0) {
	  add_nops++;		// Add NOPs after the mul.d.
	}
	muld_shadow--;
#else
...
#ifdef ICE9A_HW_WORKAROUND
    // Workaround the mul.d bug only if we are doing final scheduling.
    if (CG_muld == 1 &&
	!HBS_Before_LRA()) {
      muld_shadow--;
      if (( priority_fn->Is_Fwd_Schedule() && OP_is_like_muld(cur_op)) || 
	  (!priority_fn->Is_Fwd_Schedule() && OP_disrupts_muld(cur_op))) {
	muld_shadow = 3;
      }
    }
#endif
  ...
  BOOL ok = (BB_length(bb) == VECTOR_count(_sched_vector));

#ifdef KEY
  // _sched_vector may have extra copies.
  if (HBS_Relax_Reganti())
    ok |= (VECTOR_count(_sched_vector) >= BB_length(bb));
#endif

#ifdef ICE9A_HW_WORKAROUND
  // _sched_vector has extra NOPs for the mul.d bug.
  ok |= (VECTOR_count(_sched_vector) >= BB_length(bb));
#endif
...
#ifdef KEY
// Raise the priority of OP and its descendants.
static void
Raise_Priority (OP *op, INT32 new_priority, BB_OP_MAP priorities)
{
  INT32 old_priority = BB_OP_MAP32_Get(priorities, op);

  if (old_priority >= new_priority)
    return;

  BB_OP_MAP32_Set(priorities, op, new_priority);

  ARC_LIST *arcs;
  UINT16 latest_scycle = UINT16_MAX;	// "Latest" in the backward scheduling
					// sense.
  // Find the successor(s) with the latest scycle.
  for (arcs = OP_succs(op); arcs != NULL; arcs = ARC_LIST_rest(arcs)) {
    ARC *arc = ARC_LIST_first(arcs);
    OP *succ_op = ARC_succ(arc);
    if (OP_scycle(succ_op) < latest_scycle)
      latest_scycle = OP_scycle(succ_op);
  }
  // Raise the priority of all successors that start within DELTA of the latest
  // scycle.
  const int delta = 100;
  for (arcs = OP_succs(op); arcs != NULL; arcs = ARC_LIST_rest(arcs)) {
    ARC *arc = ARC_LIST_first(arcs);
    OP *succ_op = ARC_succ(arc);
    if (OP_scycle(succ_op) <= latest_scycle + delta)
      Raise_Priority(succ_op, new_priority, priorities);
  }
}

void
HB_Schedule::Retry_Block (BB *bb, BBSCH *bbsch, int scheduling_algorithm)
{
  int retry_limit = 4;
  int retry_count;
  int priority = 0;
  ...
  // Initialize OPs' priority to 0.
  BB_OP_MAP priorities = BB_OP_MAP32_Create(bb, &_hb_pool);
  FOR_ALL_BB_OPs_FWD(bb, op) {
    BB_OP_MAP32_Set(priorities, op, 0);
  }

  for (retry_count = 0; retry_count < retry_limit; retry_count++) {
    // Recreate dependence graph because it is destroyed by Schedule_Block (via
    // BB_Remove_All).   (TODO: Clean up code.)
    ...
    // Gradually raise the priority of the OPs at the end of the schedule.
    // Each time raise the priority of one additional OP whose previous
    // priority was zero.
    priority++;
    op = (scheduling_algorithm == 1) ? BB_last_op(bb) : BB_first_op(bb);
    while (op != NULL) {
      INT32 old_priority = BB_OP_MAP32_Get(priorities, op);
      if (old_priority < priority) {
	Raise_Priority(op, old_priority + 1, priorities);
      }
      if (old_priority == 0) {
	break;
      }
      op = (scheduling_algorithm == 1) ? OP_prev(op) : OP_next(op);
    }
    Schedule_Block(bb, bbsch, scheduling_algorithm, priorities);
    _max_sched = OP_scycle(BB_last_op(bb)) + 1;

    CG_DEP_Delete_Graph(bb);
  }
}

// Return TRUE if schedule is optimal.
BOOL
HB_Schedule::Is_Schedule_Optimal (BB *bb)
{
  if (BB_length(bb) <= 1)
    return TRUE;
...
#ifdef TARG_MIPS
  if (Is_Target_Mips5kf()) {
    // Schedule is optimal if either the int/ldst pipe is full, or if the fp
    // pipe is full.
    int schedule_length = OP_scycle(BB_last_op(bb)) + 1;
    int int_count = OPSCH_count - fp_count;	// number of int, ldst OPs
    return (schedule_length == int_count ||	// int/ldst pipe full
	    schedule_length == fp_count);	// fp pipe full
  } else if (Is_Target_twc9a()) {
    int schedule_length = OP_scycle(BB_last_op(bb)) + 1;
    int int_count = OPSCH_count - fp_count - memory_count;
    return (schedule_length == int_count ||	// int pipe full
	    schedule_length == fp_count ||	// fp pipe full
	    schedule_length == memory_count);	// ldst pipe full
  }
#endif

  return FALSE;		// Not optimal, or don't know.
}
#endif
...
HB_Schedule::Schedule_BB (BB *bb, BBSCH *bbsch, int scheduling_algorithm)
{
  ...
  // HBS_RELAX_REGANTI currently supported only for backward scheduling.
  if (scheduling_algorithm != 0) {
    _hbs_type &= ~HBS_RELAX_REGANTI;
  }
#endif
...
#else
#ifdef TARG_MIPS
	  // After delay slot filling, branch OPs are no longer at the bottom
	  // of the BB.
	  this->HBS_No_Reorder() ? NO_CONTROL_ARCS :
...
#ifdef KEY
      if (this->HBS_No_Reorder()) {
	Add_No_Reorder_Arcs(bb);
      }
#endif
      ...
      if (HBS_Retry() &&
	  BB_length(bb) > 20) {
	Retry_Block (bb, bbsch, scheduling_algorithm);
      }
#endif
    }
