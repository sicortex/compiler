...
/* routines for adding OMP trace points */
static WN *Generate_Call_Shell(char *name, TYPE_ID rtype, INT32 argc);
static inline void Add_EnterExit_OMP_Trace(char *tp_enter, char *tp_exit, 
                                           WN *wn, WN *block);
static inline void Add_BeginEnd_OMP_Trace(char *tp_begin, char *tp_end, 
                                          WN *block);
static WN *Is_OMP_Trace_Construct(WN *wn);
/* ------------------------------------ */
...
//-------------------------------------------------------------------
// NAME: Generate_Call_Shell
// FUNCTION: helper routine for generating a runtime call.
//-------------------------------------------------------------------

static WN *
Generate_Call_Shell( char *name, TYPE_ID rtype, INT32 argc )
{
...
// Name: Add_EnterExit_OMP_Trace
// Function: adds an enter and exit trace point around a WN node
static inline void Add_EnterExit_OMP_Trace(char *tp_enter, char *tp_exit, 
                                           WN *wn, WN *block)
{
  ...
  WN_INSERT_BlockBefore(block, wn, wn_enter);
  WN_INSERT_BlockAfter(block, wn, wn_exit);
}


// Name: Add_BeginEnd_OMP_Trace
// Function: adds a begin and end trace point within input block
static inline void Add_BeginEnd_OMP_Trace(char *tp_begin, char *tp_end, 
                                          WN *block)
{
  Is_True(Enable_Omp_Trace, 
  ...
  WN_INSERT_BlockFirst(block, wn_begin);
  WN_INSERT_BlockLast(block, wn_end);
}


// Name: Add_BeginEnd_OMP_Sections_TP
// Function: adds a begin and end trace point inside sections region block
static void Add_BeginEnd_OMP_Sections_TP(WN *block)
{
  WN *wn_begin;
  WN *wn_end;
  ...
  if (!sect_node || WN_opcode(sect_node) != OPC_PRAGMA ||
      ((WN_PRAGMA_ID)WN_pragma(sect_node) != WN_PRAGMA_SECTION)) {
    wn_begin = Generate_Call_Shell( "__pat_tp_omp_section_begin", 
        MTYPE_V, 0);
    WN_INSERT_BlockFirst(block, wn_begin);
    in_section = TRUE;
  } 

  /* add trace points to found sections */
  while (sect_node) {
    WN *next = WN_next(sect_node);
    if (WN_opcode(sect_node) == OPC_PRAGMA &&
        ((WN_PRAGMA_ID)WN_pragma(sect_node) == WN_PRAGMA_SECTION)) {
      if (in_section) {
        wn_end = Generate_Call_Shell( "__pat_tp_omp_section_end", 
            MTYPE_V, 0);
        WN_INSERT_BlockBefore(block, sect_node, wn_end); 
      }
      wn_begin = Generate_Call_Shell( "__pat_tp_omp_section_begin", 
          MTYPE_V, 0);
      WN_INSERT_BlockAfter(block, sect_node, wn_begin); 
      in_section = TRUE;
    }
    sect_node = next;
  }
...
static WN *Translate_OMP_to_MP(WN *wn)
{
  ...
  if (Enable_Omp_Trace) { /* add begin/end trace */
    WN *omp_tp;
    WN *region_body;
    WN *sect_node, next_sect_node;
    if (omp_tp = Is_OMP_Trace_Construct(wn)) {
      switch (WN_pragma(omp_tp)) {
        case WN_PRAGMA_PARALLEL_BEGIN:
          region_body = WN_region_body(wn);
          Add_BeginEnd_OMP_Trace("__pat_tp_omp_parallel_begin",
                                 "__pat_tp_omp_parallel_end",
                                 region_body);
          break;
        case WN_PRAGMA_PWORKSHARE_BEGIN:
          region_body = WN_region_body(wn);
          Add_BeginEnd_OMP_Trace("__pat_tp_omp_workshare_begin",
                                 "__pat_tp_omp_workshare_end",
                                 region_body);
          break;
        case WN_PRAGMA_SINGLE_PROCESS_BEGIN:
          region_body = WN_region_body(wn);
          Add_BeginEnd_OMP_Trace("__pat_tp_omp_single_begin",
                                 "__pat_tp_omp_single_end",
                                 region_body);
          break;
        case WN_PRAGMA_PSECTION_BEGIN:
          region_body = WN_region_body(wn);
          Add_BeginEnd_OMP_Sections_TP(region_body);
          break;
      }
    }   
  }
...
*/
  ...
  if (opcode == OPC_BLOCK) {
    ...
    while (kid) {
      ...
      if (Enable_Omp_Trace) { /* add enter/exit trace */
        WN *omp_tp;
        if (omp_tp = Is_OMP_Trace_Construct(kid)) {
          switch (WN_pragma(omp_tp)) {
            case WN_PRAGMA_PARALLEL_BEGIN:
              Add_EnterExit_OMP_Trace("__pat_tp_omp_parallel_enter",
                  "__pat_tp_omp_parallel_exit", kid, wn);
              break;
            case WN_PRAGMA_PWORKSHARE_BEGIN:
              Add_EnterExit_OMP_Trace("__pat_tp_omp_workshare_enter",
                  "__pat_tp_omp_workshare_exit", kid, wn);
              break;
            case WN_PRAGMA_PDO_BEGIN:
              Add_EnterExit_OMP_Trace("__pat_tp_omp_loop_enter",
                  "__pat_tp_omp_loop_exit", kid, wn);
              break;
            case WN_PRAGMA_SINGLE_PROCESS_BEGIN:
              Add_EnterExit_OMP_Trace("__pat_tp_omp_single_enter",
                  "__pat_tp_omp_single_exit", kid, wn);
              break;
            case WN_PRAGMA_PSECTION_BEGIN:
              Add_EnterExit_OMP_Trace("__pat_tp_omp_sections_enter",
                  "__pat_tp_omp_sections_exit", kid, wn);
              break;
          }
        }   
      }
  ...
  for (WN* wn = wn_first_pragma; wn != NULL; wn = WN_next(wn)) { 
	...
	break;
      case WN_PRAGMA_SCHEDTYPE_PSEUDOLOWERED:
        FmtAssert(FALSE, ("Should not have seen this pragma yet."));
      } 
    }
...
// Is this a parallel begin, workshare begin, pdo begin, single process begin,
// or psection begin?
  ...
  if (pragmas) {
    ...
    while (pragma) {
      if (WN_opcode(pragma) == OPC_PRAGMA) {
        WN_PRAGMA_ID pragma_id = (WN_PRAGMA_ID)WN_pragma(pragma);
        if (pragma_id == WN_PRAGMA_PARALLEL_BEGIN ||
            pragma_id == WN_PRAGMA_PWORKSHARE_BEGIN ||
            pragma_id == WN_PRAGMA_PDO_BEGIN ||
            pragma_id == WN_PRAGMA_SINGLE_PROCESS_BEGIN ||
            pragma_id == WN_PRAGMA_PSECTION_BEGIN) 
          return pragma;
...
static void Atomic_Using_Critical(WN *atomic, WN *store)
{
  ...
  char name[26];
  strcpy(name, "__OMP_CRITICAL_ATOMIC_??");  // default
  switch (WN_desc(store)) {
    case MTYPE_I1:  name[22] = 'I';  name[23] = '1';  break; 
    case MTYPE_I2:  name[22] = 'I';  name[23] = '2';  break;
    case MTYPE_I4:  name[22] = 'I';  name[23] = '4';  break;
    case MTYPE_I8:  name[22] = 'I';  name[23] = '8';  break;
    case MTYPE_U1:  name[22] = 'U';  name[23] = '1';  break;
    case MTYPE_U2:  name[22] = 'U';  name[23] = '2';  break;
    case MTYPE_U4:  name[22] = 'U';  name[23] = '4';  break;
    case MTYPE_U8:  name[22] = 'U';  name[23] = '8';  break;
    case MTYPE_F4:  name[22] = 'F';  name[23] = '4';  break;
    case MTYPE_F8:  name[22] = 'F';  name[23] = '8';  break;
    case MTYPE_FQ:  name[22] = 'F';  name[23] = 'Q';  break;
    case MTYPE_C4:  name[22] = 'C';  name[23] = '4';  break;
    case MTYPE_C8:  name[22] = 'C';  name[23] = '8';  break;
    case MTYPE_CQ:  name[22] = 'C';  name[23] = 'Q';  break;
    case MTYPE_F16:
      name[22] = 'F';  name[23] = '1';  name[24] = '6';  name[25] = '\0';
      break;
    case MTYPE_C16:
      name[22] = 'C';  name[23] = '1';  name[24] = '6';  name[25] = '\0';
      break;
  }
