...
Ldid_from_mtype( MTYPE mtype )
{
  switch ( mtype ) {
    ...
    case MTYPE_F16:     return OPC_F16F16LDID;
    case MTYPE_C4:	return OPC_C4C4LDID;
    case MTYPE_C8:	return OPC_C8C8LDID;
    case MTYPE_C16:     return OPC_C16C16LDID;
...
Mtype_from_mtype_class_and_size( INT mtype_class, INT bytes )
{
  ...
  if ( mtype_class & MTYPE_CLASS_VECTOR ) {
    if ( ( mtype_class & MTYPE_CLASS_SVECTOR ) == MTYPE_CLASS_SVECTOR &&
	 bytes == 4 ) {
      if ( mtype_class & MTYPE_CLASS_FLOAT ) return MTYPE_V8F4;
      if ( mtype_class & MTYPE_CLASS_INTEGER ) return MTYPE_V8I4;
    }
    FmtAssert( FALSE,
	       ("Mtype_from_mtype_class_and_size: unknown type"));
  } else
...
Ldid_from_mtype_class_and_size( INT mtype_class, INT bytes )
{
  ...
  if ( mtype_class & MTYPE_CLASS_VECTOR ) {
    if ( ( mtype_class & MTYPE_CLASS_SVECTOR ) == MTYPE_CLASS_SVECTOR &&
	 bytes == 4 ) {
      if ( mtype_class & MTYPE_CLASS_FLOAT ) return OPC_V8F4V8F4LDID;
      if ( mtype_class & MTYPE_CLASS_INTEGER ) return OPC_V8I4V8I4LDID;
    }
    FmtAssert( FALSE,
	       ("Ldid_from_mtype_class_and_size: unknown type"));
  } else
...
Stid_from_mtype_class_and_size( INT mtype_class, INT bytes )
{
  ...
  if ( mtype_class & MTYPE_CLASS_VECTOR ) {
    if ( ( mtype_class & MTYPE_CLASS_SVECTOR ) == MTYPE_CLASS_SVECTOR &&
	 bytes == 4 ) {
      if ( mtype_class & MTYPE_CLASS_FLOAT ) return OPC_V8F4STID;
      if ( mtype_class & MTYPE_CLASS_INTEGER ) return OPC_V8I4STID;
    }
    FmtAssert( FALSE,
	       ("Stid_from_mtype_class_and_size: unknown type"));
  } else
...
#ifdef TARG_MIPS
    TYPE_ID type = sym->Mtype();
    if (type == MTYPE_V8F4 || type == MTYPE_V8I4) bytes = 4;
#endif
