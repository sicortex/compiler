...
void POINTS_TO::Print(FILE *fp) const
{
  ...
  if (pr_ofst) {
    ...
    case OFST_IS_INVALID:
      ...
      break;
    case OFST_IS_FIXED:
      fprintf(fp, "byte ofst is %" SCNd64 ", ", Byte_Ofst());
      if (Bit_Size() != 0)
  ...
  if (pr_size) {
    fprintf(fp, "byte size is %" SCNd64 ", ", Byte_Size());
    if (Bit_Size() != 0)
      fprintf(fp, "bit size is %d, ", Bit_Size());
  }
  fprintf(fp, "per-PU class %" PRIuPTR " , ", Alias_class());
  fprintf(fp, "global class %" PRIuPTR " , ", Ip_alias_class());
