		 ...
		 EBO_TN_INFO **pred_tninfo_p = NULL)
{
  ...
  // Assume the TN is not available if it is in an x86 special-purpose
  // register, since the TN will have to be deallocated from the register if an
  // intervening OP uses that register.  (TODO: Analyze the intervening OPs.
  // Return FALSE only if such an intervening OP exists.)  (bug 14565)
  if (!EBO_in_peep) {
    TN *tn = pred_tninfo->local_tn;
...
BOOL Special_Sequence( OP *op, TN **opnd_tn, EBO_TN_INFO **opnd_tninfo )
{
  ...
  // On x86-64, 32-bit OPs always use 32-bit operands.  The upper 32 bits of
  // the 64-bit register opernad is ignored.  As a result, don't need to
  // zero-extend the operand first.  Change:
  //   TN101 = mov32 TN100		; TN100 is size 8.  TN101 is size 4.
  //   TN102 = add32 TN101 ...		; 32-bit operands
  // to:
  //   TN102 = add32 TN100 ...		; use the 64-bit TN directly
  if (!EBO_in_peep &&			// Don't break x86-style property.
      OP_results(op) == 1 &&
      OP_result_reg_class(op, 0) == ISA_REGISTER_CLASS_integer &&
      (OP_result_size(op, 0) == 32 ||
       OP_code(op) == TOP_movslq)) {
    int i;
    OP *new_op = NULL;
    for (i = 0; i < OP_opnds(op); i++) {
      TN *opnd = OP_opnd(op, i);
      if (TN_is_register(opnd)) {
	EBO_TN_INFO *mov32_opnd0_tninfo = NULL;
	EBO_TN_INFO *tninfo = opnd_tninfo[i];
	OP *pred_op = (tninfo == NULL) ? NULL : tninfo->in_op;
	if (pred_op &&
	    OP_code(pred_op) == TOP_mov32 &&
	    Pred_Opnd_Avail(op, tninfo, 0, &mov32_opnd0_tninfo)) {
	  // Substitute only if the replacement TN does not have an assigned
	  // register, since we don't want to lengthen the live-range of real
	  // registers.  Furthermore, this avoids the case where OP itself may
	  // need that real register for another of its operands, such as ecx
	  // for shl32.
	  if (!has_assigned_reg(mov32_opnd0_tninfo->local_tn)) {
	    if (new_op == NULL)
	      new_op = Dup_OP(op);
	    TN *new_opnd = OP_opnd(pred_op, 0);
	    Set_OP_opnd(new_op, i, new_opnd);
	  }
	}
      }
    }
    if (new_op != NULL) {
      ...
      BB_Insert_Op_After(OP_bb(op), op, new_op);
      if (EBO_Trace_Optimization)
	fprintf(TFile, "zero-extend not needed");
      return TRUE;
    }
  }
...
BOOL
Special_Asm_Sequence (OP *op, TN **opnd_tn, EBO_TN_INFO **opnd_tninfo)
{
  int i;
  ASM_OP_ANNOT *asm_info = (ASM_OP_ANNOT *) OP_MAP_Get(OP_Asm_Map, op);
  for (i = 0; i < OP_opnds(op); i++) {
    // Replace:
    //   tn = lea32 offset(base)
    //   asm tn ...
    //
    // with:
    //   tn = lea32 offset(base)
    //   asm base ...		// offset stored in ASM_OP_opnd_offset
    //
    // This reduces register usage since base is often %rsp.  Bug 14797.

    if (ASM_OP_opnd_memory(asm_info)[i] &&
	ASM_OP_opnd_offset(asm_info)[i] == NULL) {
      const EBO_TN_INFO *tninfo = opnd_tninfo[i];
      OP *lea32_op = tninfo == NULL ? NULL : tninfo->in_op;
      EBO_TN_INFO *lea32_opnd0_tninfo = NULL;
      if (lea32_op != NULL &&
	  OP_code(lea32_op) == TOP_lea32 &&
	  Pred_Opnd_Avail(op, opnd_tninfo[i], 0, &lea32_opnd0_tninfo)) {
	// Change the ASM base register.
	Set_OP_opnd(op, i, OP_opnd(lea32_op, 0));
	dec_ref_count(opnd_tninfo[i]);
	inc_ref_count(lea32_opnd0_tninfo);
	opnd_tninfo[i] = lea32_opnd0_tninfo;
	// The ASM operand now has an offset.
	ASM_OP_opnd_offset(asm_info)[i] = OP_opnd(lea32_op, 1);
      }
    }
  }
  return FALSE;
			  ...
			  TN** index, TN** offset, TN** scale, TN** base )
{
  ...
  *index = *offset = *base = *scale = NULL;
  ...
  /* Bug 14488: under -mcmodel=kernel, if the offset is with respect
     to a symbol or section name, check for a smaller offset size,
...
static Addr_Mode_Group Addr_Mode_Group_Table[] = {
  ...
  /*AVX 256-bits*/
  {TOP_vaddpd_f256_ofloat_float_float, TOP_vaddpd_f256_ofloat_float_base64_simm32, 
   TOP_vaddpd_f256_ofloat_float_base64_index64_uimm8_simm32, TOP_vaddpd_f256_ofloat_float_index64_uimm8_simm32, TOP_UNDEFINED},
  {TOP_vaddps_f256_ofloat_float_float, TOP_vaddps_f256_ofloat_float_base64_simm32,
   TOP_vaddps_f256_ofloat_float_base64_index64_uimm8_simm32, TOP_vaddps_f256_ofloat_float_index64_uimm8_simm32, TOP_UNDEFINED},
