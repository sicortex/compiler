...
COMP_UNIT::Verify_IR(CFG *cfg, CODEMAP *htable, INT which)
{
  ...
  FOR_ALL_ELEM (bb, cfg_iter, Init(cfg)) {	// go through all BBs
    ...
    FOR_ALL_NODE(stmt, stmt_iter, Init())	// go through all STMTs
	  ...
	  if (stmt->Lhs() == NULL || stmt->Lhs() == UNDEFINED_PTR ||
	    ...
	    fprintf(TFile,"*** Verify_IR %d, BB%" PRIdPTR ", %s null Istr_base\n",
  ...
  FOR_ALL_ELEM (bb, cfg_iter, Init(cfg)) {	// go through all BBs
	...
	if (stmt->Bb() != NULL && stmt->Bb() != UNDEFINED_PTR)
	  sprintf(str,"BB%" PRIdPTR "",stmt->Bb()->Id());
	else
	  strcpy(str,"<null>");
	fprintf(TFile,
		"*** Verify_IR %d, BB%" PRIdPTR ", stmtrep bb pointer is wrong, %s\n",
		which,bb->Id(),str);
	stmt->Print(TFile);
	...
	FOR_ALL_NODE( cnode, chi_iter, Init(chi_list)) {
	  if (cnode->Live()) {
	    ...
	    if (cnode->RESULT() == NULL || cnode->RESULT() == UNDEFINED_PTR) {
	      fprintf(TFile,"*** Verify_IR %d, BB%" PRIdPTR ", chi RESULT is null\n",
		      which,bb->Id());
	      continue;
	    }
	    if (cnode->OPND() == NULL || cnode->OPND() == UNDEFINED_PTR) {
	      fprintf(TFile,"*** Verify_IR %d, BB%" PRIdPTR ", chi OPND is null\n",
		      which,bb->Id());
	      continue;
	    }
	    ...
	    if (!found_r)
	      fprintf(TFile,"*** Verify_IR %d, BB%" PRIdPTR ", chi RESULT not right\n",
		      which,bb->Id());
	    if (!found_o)
	      fprintf(TFile,"*** Verify_IR %d, BB%" PRIdPTR ", chi OPND not right\n",
		      which,bb->Id());
...
Verify_version_istore(STMTREP *stmt, BB_NODE *bb, CODEMAP *htable, OPT_STAB *opt_stab)
{
  ...
  FmtAssert(FALSE, ("CK_IVAR has no chi of vsym."));
}
