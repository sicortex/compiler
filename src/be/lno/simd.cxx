...
static BOOL is_vectorizable_op (OPERATOR opr, TYPE_ID rtype, TYPE_ID desc) {
  ...
  case OPR_LT: case OPR_GT: case OPR_LE: case OPR_GE:
    if ((MTYPE_is_float(desc) || 
         MTYPE_is_integral(desc) && MTYPE_byte_size(desc) <= 4) && 
    ...
    if(WN_operator(wn) == OPR_ARRAY && 
      ...
      for (INT i = 0; i < aa->Num_Vec(); i ++) {
       ...
       if (ok && PU_src_lang(Get_Current_PU()) == PU_F90_LANG)
         ...
         if(!ok){ 
           if(in_simd && (debug || LNO_Simd_Verbose || LNO_Lno_Verbose)){
...
static BOOL Is_Well_Formed_Simd ( WN* wn, WN* loop)
{
  ...
  if (WN_operator(wn) == OPR_SELECT) {
    WN *compare_wn = kid0;
    if (!OPCODE_is_compare(WN_opcode(compare_wn)))
      return FALSE;
    if (!MTYPE_is_float(WN_desc(compare_wn)) &&
        !MTYPE_is_integral(WN_desc(compare_wn)))
      return FALSE;
    if (MTYPE_is_integral(WN_desc(compare_wn)) &&
        MTYPE_byte_size(WN_desc(compare_wn)) == 8)
      return FALSE;
    INT select_size = MTYPE_byte_size(WN_rtype(wn));
    if (WN_operator(WN_kid0(compare_wn)) == OPR_ILOAD &&
        MTYPE_byte_size(WN_desc(WN_kid0(compare_wn))) != select_size)
      return FALSE;
    if (WN_operator(WN_kid1(wn)) == OPR_ILOAD &&
        MTYPE_byte_size(WN_desc(WN_kid1(wn))) != select_size)
    kid0 = WN_kid1(wn);
    kid1 = WN_kid2(wn);
  }

   //bug 14810 and bug 14811: For comparison vectorization, CG can only handle the case
   // in conditional moves. So here we restrict the comparison vectorization only when 
   // SELECT is involved. 
  ...
  WN* wn, SCALAR_REF_STACK* simd_ops, MEM_POOL *pool, WN *loop)
{
  ...
  if (WN_operator(wn) == OPR_ISTORE && 
      OPERATOR_is_compare(WN_operator(WN_kid0(wn)))) {
    if (MTYPE_byte_size(WN_desc(wn)) != MTYPE_byte_size(WN_desc(WN_kid0(wn)))) {
      Report_Non_Vectorizable_Op(wn);
      return FALSE;	
    }
  }
...
static WN *Simd_Vectorize_Constants(WN *const_wn,//to be vectorized 
                                    ...
                                    WN *simd_op,
                                    INT num_simd_op) //const_wn's parent
{
   ...
   if (WN_desc(type_node) == MTYPE_V)
       ...
       type = WN_desc(type_node);
   if (WN_operator(simd_op) == OPR_PARM &&
          WN_operator(type_node) == OPR_INTRINSIC_OP &&
          WN_intrinsic(type_node) == INTRN_SUBSU2) {
        type = WN_desc(LWN_Get_Parent(type_node));
    }
  ...
  if (WN_operator(simd_op) != OPR_CVT && WN_operator(simd_op) != OPR_TRUNC){
    for(INT kid=0; kid<WN_kid_count(simd_op); kid++){
      ...
      if(WN_desc(operand) != MTYPE_V && !MTYPE_is_vector(WN_desc(operand)))
         WN_set_desc(operand, Mtype_TransferVector(vmtype, WN_desc(operand)));
      if(WN_rtype(operand) != MTYPE_V && !MTYPE_is_vector(WN_rtype(operand)))
        WN_set_rtype(operand, Mtype_TransferVector(vmtype, WN_rtype(operand)));
    }//end kids

    if(OPCODE_is_compare(WN_opcode(simd_op))){
      WN_set_rtype (simd_op, Mtype_TransferVector(vmtype, WN_rtype(simd_op)));
      WN_set_desc(simd_op, Mtype_TransferVector(vmtype, WN_desc(simd_op)));
...
//START: Vectorization Module 
  ...
  for (INT i=vec_simd_ops->Elements()-1; i >= 0; i--){
    ...
    istore=LWN_Get_Parent(simd_op);
    //bug 14738: For a cmp or cvt, the kid's simd type should be controled
    //by the cmp or cvt, not istore (their parent). 
    WN *kid_type_control_node= istore;
    if(OPCODE_is_compare(WN_opcode(simd_op)) || 
        WN_operator(simd_op) == OPR_CVT      ||
        WN_operator(simd_op) == OPR_TRUNC)
       kid_type_control_node = simd_op;    
	...
	if (WN_operator(inv_node) == OPR_REPLICATE) continue;
	  ...
	  if(MTYPE_is_vector(WN_rtype(inv_node))) continue; //next kid
          ...
          inv_node = Simd_Vectorize_Constants(inv_node, kid_type_control_node, simd_op , i);
        }
      }
