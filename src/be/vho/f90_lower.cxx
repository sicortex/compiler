				...
				INT *flag, TY_IDX *ty)
{
   ...
   *ty = (TY_IDX)0;
   ...
   while (!done) {
	 ...
	 *flag |= F90_THROUGH_ARRAY;
	 ...
	 if (WN_operator(kid) == OPR_INTCONST) {
	    ADD_PIECE(kid);
	    addr = WN_kid0(addr);
	    if (trace_dependence) fprintf(TFile,"  Analyze_one_address (Add %" SCNd64 ")\n",WN_const_val(kid));
	    continue;
	 }
	 kid = WN_kid0(addr);
	 if (WN_operator(kid) == OPR_INTCONST) {
   ...
   if (trace_dependence) {
      if (*sym) {
	 fprintf(TFile,"sym: %s, base_offset %" SCNd64 ", const_offset %" SCNd64 ", size %" SCNd64 "\n",
...
static DIR_FLAG Analyze_index(WN *i1, WN *i2) {
   ...
   if (trace_dependence) {
      if (e_constant) fprintf(TFile,"E = %" SCNd64 "\n",e);
      if (l_constant) fprintf(TFile,"L = %" SCNd64 "\n",l);
      if (s_constant) fprintf(TFile,"S1,S2 = %" SCNd64 " %" SCNd64 "\n",s1,s2);
   }
