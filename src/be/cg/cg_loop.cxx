...
#ifdef KEY
void 
CG_LOOP_Copy_Loop_Info (OP *op_tgt, const OP *op_src) {
  FmtAssert (   OP_code (op_tgt) == OP_code (op_src)
             && OP_results (op_tgt) == OP_results (op_src)
             && OP_opnds (op_tgt) == OP_opnds (op_src),
             ("OPs are not of the same type"));

  // Check if the source OP has even a loop info attached. 
  _CG_LOOP_INFO *info_src = _CG_LOOP_info (op_src);
  FmtAssert (info_src, 
             ("Can't copy the loop info, because the source OP" \
              "has no loop info attached."));

  // Check if the target OP has already a loop info attached
  // and create one if necessary
  _CG_LOOP_INFO *info_tgt   = _CG_LOOP_info (op_tgt);
  INT            sizeof_info = _CG_LOOP_info_sizeof (op_tgt);

  if (info_tgt == 0) {
    info_tgt = free_loop_info;
    if (info_tgt && OP_opnds (op_tgt) <= OP_MAX_FIXED_OPNDS) {
      // Since we don't track the size of the _CG_LOOP_INFO structs on
      // the free list, don't use one for a variable operand OP uses 
      // more than OP_MAX_FIXED_OPNDS, instead allocate a new one. We 
      // can however guarantee that all _CG_LOOP_INFO structs allocate 
      // are large enough to handle all fixed operand OPs, so anything
      // on the free list will do in that case.
      free_loop_info = next_free_loop_info (info_tgt);
    } 
    else {
      info_tgt = (_CG_LOOP_INFO *) MEM_POOL_Alloc (&MEM_phase_nz_pool,
                                                   sizeof_info);
    }
  }
  
  memcpy (info_tgt, info_src, sizeof_info);
  OP_MAP_Set(_CG_LOOP_info_map, op_tgt, info_tgt);
...
void CG_LOOP::Build_CG_LOOP_Info(BOOL single_bb)
{
  ...
  if (Post_ntimes() > 0) {
    ...
    CGTARG_Analyze_Compare(br_op, &opnd0, &opnd1, &compare_op);

    OP *op;
    BOOL opnd0_is_iv = FALSE;
    BOOL opnd1_is_iv = FALSE;
    FOR_ALL_BB_OPs(body, op) {
      if (CGTARG_Is_OP_Addr_Incr(op)) {
	if (OP_result(op, 0) == opnd0) {
	  opnd0_is_iv = TRUE;
	  iv_tn = opnd0;
	}
	if (OP_result(op, 0) == opnd1) {
	  opnd1_is_iv = TRUE;
	  iv_tn = opnd1;
	}
      }
    }
    FmtAssert(!opnd0_is_iv || !opnd1_is_iv,
	      ("Build_CG_LOOP_Info: unexpected iv tn"));

    // Trip count TN is not in the compare-and-branch OP.  The loop is not in a
    // form that the post-remainder code can handle.  Add support as needed.
    if (iv_tn == NULL) {
      Set_post_ntimes(0);
    } else {
      Is_True(TN_is_global_reg(iv_tn),
	      ("Build_CG_LOOP_Info: ivar TN not a global_reg"));
    }
  }
#endif
...
#ifdef KEY
	      // If adding post-epilogue remainder loop, make TN live-out of
	      // the loop body so that it is available to the remainder loop.
	      if (Post_ntimes() > 0) {
		Backpatch_Post_Remainder_Add(tn);
	      }
#endif
	    }
  ...
  if (iv_tn != NULL &&
      !TN_is_dedicated(iv_tn) &&
      !CG_LOOP_Backpatch_Find_Non_Body_TN(epilog, iv_tn, 0)) {
    Backpatch_Post_Remainder_Add(iv_tn);
  }
#endif
...
#ifdef KEY
  post_remainder_loophead = NULL;
  post_ntimes = 0;
  int vec_size = BB_length(body) + 8;	// increase size as necessary
  backpatch_post_remainder_TNs = VECTOR_Init(vec_size, &MEM_phase_nz_pool);
#endif
...
#ifdef KEY
void CG_LOOP_Backpatch_Add(BB *bb, CG_LOOP_BACKPATCH *bp)
/* -----------------------------------------------------------------------
 * See "cg_loop.h" for interface.
 * -----------------------------------------------------------------------
 */
{
  ...
  if (bb == CG_LOOP_prolog) {
    bp->next = prolog_backpatches;
    prolog_backpatches = bp;
  }
  else {
    bp->next = epilog_backpatches;
    epilog_backpatches = bp;
  }
...
#ifdef KEY
void CG_LOOP_Backpatch_Clear(BB *bb)
/* -----------------------------------------------------------------------
 * See "cg_loop.h" for interface.
 * -----------------------------------------------------------------------
 */
{
  ...
  if (bb == CG_LOOP_prolog) {
    prolog_backpatches = 0;
  }
  else {
    epilog_backpatches = 0;
  }
...
void CG_LOOP_Backpatch_Trace(BB *bb, CG_LOOP_BACKPATCH *bp, FILE *f)
/* -----------------------------------------------------------------------
 * See "cg_loop.h" for interface.
 * -----------------------------------------------------------------------
 */
{
  ...
  if (bp) {
    if (bb == CG_LOOP_epilog) {
      fprintf(f, "<bp>   TN%d <- TN%d[%d]\n", TN_number(bp->non_body_tn),
	      TN_number(bp->body_tn), bp->omega);
    } else {
      fprintf(f, "<bp>   TN%d[%d] %s TN%d\n", TN_number(bp->body_tn),
	      bp->omega, bb == CG_LOOP_prolog ? "<-" : "",
	      TN_number(bp->non_body_tn));
    }
  } else if (bb) {
    BOOL prolog = bb == CG_LOOP_prolog;
    fprintf(f, "<bp> %slog backpatches:\n", prolog ? "pro" : "epi");
    for (bp = prolog ? prolog_backpatches : epilog_backpatches; bp;
	 bp = bp->next)
      CG_LOOP_Backpatch_Trace(bb, bp, f);
    fprintf(f,"\n");
  } else {
    CG_LOOP_Backpatch_Trace(CG_LOOP_prolog, NULL, f);
    CG_LOOP_Backpatch_Trace(CG_LOOP_epilog, NULL, f);
  }
}
 ...
 */
{
  ...
  if (!unroll_fully) {
    ...
    if (cl->Has_post_remainder_loop()) {
      ...
      // Find the increment.
      FOR_ALL_BB_OPs(unrolled_body, op1) {
	TOP top = OP_code(op1);
	if ((top == TOP_addi || top == TOP_addiu ||
	     top == TOP_daddi || top == TOP_daddiu)) {
	  if (OP_result(op1, 0) == tn1) {
	    FmtAssert(incr_op == NULL,
		      ("Unroll_Replicate_Body: found multiple incr OP"));
	    bound_tn = tn2;
	    incr_op = op1;
	  } else if (OP_result(op1, 0) == tn2) {
	    FmtAssert(incr_op == NULL,
		      ("Unroll_Replicate_Body: found multiple incr OP"));
	    bound_tn = tn1;
	    incr_op = op1;
      ...
      FmtAssert(incr_op != NULL,
		("Unroll_Replicate_Body: cannot find incr OP"));

      // Reduce bound by "increment * number of post-remainder iterations".
      TN *incr_tn = OP_opnd(incr_op, 1);
      Is_True(TN_is_constant(incr_tn) && TN_has_value(incr_tn),
	      ("Unroll_Replicate_Body: cannot find incr OP"));
      TN *new_bound_tn = Build_TN_Like(bound_tn);
      Exp_OP2(TN_size(bound_tn) == 4 ? OPC_U4SUB: OPC_U8SUB,
		      new_bound_tn, bound_tn,
		      Gen_Literal_TN(cl->Post_ntimes() * TN_value(incr_tn),
				     TN_size(incr_tn)),
		      pre_prolog_ops);
      Set_OP_opnd(op, bound_tn == tn1 ? 0 : 1, new_bound_tn);
#else
      FmtAssert(FALSE, ("Unroll_Replicate_Body: NYI for this arch"));
#endif
    }
#endif	// KEY
  }
...
#ifdef KEY
void
CG_LOOP::Backpatch_Post_Remainder_Add (TN *new_tn)
{
  int i;

  for (i = 0; i < VECTOR_count(backpatch_post_remainder_TNs); i++) {
    TN *tn = (TN *) VECTOR_element(backpatch_post_remainder_TNs, i);
    if (tn == new_tn)
      return;
  }
  VECTOR_Add_Element(backpatch_post_remainder_TNs, new_tn);
}

void
CG_LOOP::Determine_Post_Ntimes()
{ 
  LOOP_DESCR *loop = Loop();
  TN *trip_count_tn = CG_LOOP_Trip_Count(loop);

  if (!CG_LOOP_convert_prefetch_to_load)
    return;

  if (CG_LOOP_post_remainder_loop > 0) {
    // Mainly for debugging purpose.
    Set_post_ntimes(CG_LOOP_post_remainder_loop);
  } else {
#ifdef TARG_MIPS
    if (Is_Target_Mips5kf()) {
      const int L1_line_size = 32;
      UINT32 stride = 0;
      BB *loop_body = LOOP_DESCR_loophead(loop);
      OP *op;

      for (op = BB_first_op(loop_body); op != NULL; op = OP_next(op)) {
	TOP opc = OP_code(op);
	if (OP_prefetch(op)) {
	  UINT32 this_stride = CG_LOOP_Prefetch_Stride(op);
      ...
      if (stride > 0) {
	int remainder_iters = (LNO_Prefetch_Ahead * L1_line_size) / stride;
	Set_post_ntimes(remainder_iters);
      }
    }
#endif
  }
...
// make a copy of the loop to serve as the post-remainder loop, but don't
// insert the loop into the BB list yet.  Also, add the TNs used by the
// post-remainder loop to the backpatch.
static void
Unroll_Make_Post_Remainder_Loop (CG_LOOP &cg_loop)
{
  UINT32 post_ntimes = cg_loop.Post_ntimes();

  if (post_ntimes == 0 ||
      cg_loop.Unroll_fully())
    return;

  // Don't add post-remainder loop if the trip count is too small.
  LOOPINFO *info = LOOP_DESCR_loopinfo(cg_loop.Loop());
  TN *trip_count_tn = info ? LOOPINFO_trip_count_tn(info) : NULL;
  if (TN_is_constant(trip_count_tn)) {
    INT32 const_trip_count = TN_value(trip_count_tn) > INT32_MAX ?
			       INT32_MAX : TN_value(trip_count_tn);
    const INT32 threshold = 4;	// Heuristic, must be >= 0.
    if ((int) const_trip_count - (int) post_ntimes < threshold)
      return;
  }

  if (Get_Trace(TP_CGLOOP, 4)) {
    #pragma mips_frequency_hint NEVER
    CG_LOOP_Trace_Loop(cg_loop.Loop(), "before Unroll_Make_Post_Remainder");
  }

  LOOP_DESCR *loop = cg_loop.Loop();
  BB *orig_bb = LOOP_DESCR_loophead(loop);
  BB *new_bb = Gen_BB_Like(orig_bb);
  OP *op;

  LOOPINFO *new_info = TYPE_P_ALLOC(LOOPINFO);
  WN *wn = WN_COPY_Tree(LOOPINFO_wn(info));
  LOOPINFO_wn(new_info) = wn;
  LOOPINFO_srcpos(new_info) = LOOPINFO_srcpos(info);
  BB_Add_Annotation(new_bb, ANNOT_LOOPINFO, new_info);

  TYPE_ID ttype = WN_rtype(WN_loop_trip(wn));
  OPCODE opc_intconst = OPCODE_make_op(OPR_INTCONST, ttype, MTYPE_V);
  WN *post_ntimes_wn = WN_CreateIntconst(opc_intconst, post_ntimes);
  WN *gt_wn = WN_GT(ttype, WN_COPY_Tree(WN_loop_trip(wn)), post_ntimes_wn);
  WN *select_wn = WN_Select(ttype, gt_wn, post_ntimes_wn,
			    WN_COPY_Tree(WN_loop_trip(wn)));
  WN_set_loop_trip(wn, select_wn);
  INT loop_count = WN_loop_trip_est(wn);
  WN_loop_trip_est(wn) = loop_count > post_ntimes ? post_ntimes : loop_count;

  FOR_ALL_BB_OPs(orig_bb, op) {
    if (OP_prefetch(op))	// Drop all prefetches.
      continue;
    OP *new_op = Dup_OP(op);
    CGPREP_Init_Op(new_op);
    Copy_WN_For_Memory_OP(new_op, op);
  ...
  unroll_xfer_annotations(new_bb, orig_bb);

  if (FREQ_Frequencies_Computed() || BB_freq_fb_based(orig_bb)) {
    float new_freq = BB_freq(orig_bb) > post_ntimes ?
		       post_ntimes : BB_freq(orig_bb);
    BB_freq(new_bb) = new_freq;
  }

  cg_loop.Set_post_remainder_loophead(new_bb);
  cg_loop.Set_has_post_remainder_loop();

  // Add TNs used by the post-remainder loop to the backpatch, so that they
  // will be live-out of the main loop and available to the post-remainder
  // loop.
  int i;
  BB *epilog = cg_loop.Epilog_start();
  for (i = 0; i < VECTOR_count(cg_loop.Backpatch_post_remainder_TNs()); i++) {
    TN *tn = (TN *) VECTOR_element(cg_loop.Backpatch_post_remainder_TNs(), i);
...
{
  if (!cg_loop.Has_post_remainder_loop())
    return;

  BB *bb = cg_loop.Post_remainder_loophead();
  ...
  Is_True(op, ("Unroll_Insert_Post_Remainder_Loop: missing loopback branch"));
  Set_OP_opnd(op,
	      Branch_Target_Operand(op),
	      Gen_Label_TN(Gen_Label_For_BB(bb), 0));

  BB *epilog_end_bb = cg_loop.Epilog_end();
  BB *next_bb = BB_next(epilog_end_bb);
  Unlink_Pred_Succ(epilog_end_bb, next_bb);
  Chain_BBs(bb, next_bb);
  Chain_BBs(epilog_end_bb, bb);
  Link_Pred_Succ_with_Prob(epilog_end_bb, bb, 1.0);
  Link_Pred_Succ_with_Prob(bb, next_bb, 1.0 / loop_count);
  Link_Pred_Succ_with_Prob(bb, bb, (loop_count - 1.0) / loop_count);
...
void Unroll_Do_Loop(CG_LOOP& cl, UINT32 ntimes)
{
  ...
  if (TN_is_constant(trip_count_tn)) {
    ...
    if (cl.Has_post_remainder_loop()) {
      ...
      TN *new_trip_count_tn = Gen_Literal_TN(new_value, TN_size(trip_count_tn));
      trip_count_tn = new_trip_count_tn;

      ANNOTATION *an = ANNOT_Get(BB_annotations(head), ANNOT_LOOPINFO);
      LOOPINFO *info = ANNOT_loopinfo(an);
    ...
    if (cl.Has_post_remainder_loop()) {
      ...
      // the number of post remainder iterations.
      TN *new_trip_count_tn = Build_TN_Like(trip_count_tn);
      Exp_OP2(trip_size == 4 ? OPC_U4SUB : OPC_U8SUB,
	      new_trip_count_tn,
	      trip_count_tn,
	      Gen_Literal_TN(cl.Post_ntimes(), trip_size),
	      &pre_prolog_ops);
      trip_count_tn = new_trip_count_tn;
  ...
  if (cl.Has_post_remainder_loop()) {
    ...
    // Branch directly to post-remainder loop if trip count <= number of
    // post-remainder iterations.
    BB *post_remainder_loop = cl.Post_remainder_loophead();
    LABEL_IDX lbl = Gen_Label_For_BB(post_remainder_loop);
    Exp_OP3v(OPC_TRUEBR,
	     NULL,
	     Gen_Label_TN(lbl, 0),
	     trip_count_tn,
    ...
    // Insert new trip count calculation to the prolog.  Do this before calling
    // Unroll_Make_Remainder_Loop, which inserts its own OPs to the prolog.
    BB_Append_Ops(CG_LOOP_prolog, &pre_prolog_ops);
    float prob = 0.1;	// todo: calculate
    Link_Pred_Succ_with_Prob(CG_LOOP_prolog, post_remainder_loop, prob);
    extend_prolog();	// The branch ends the BB.  Create a new BB.
    Change_Succ_Prob(BB_prev(CG_LOOP_prolog), CG_LOOP_prolog, 1.0 - prob);
  }
#endif
...
#ifdef KEY	// bug 14623
    INT32 const_trip_count =
	    const_trip ? (TN_value(trip_count_tn) > INT32_MAX ?
			    INT32_MAX : TN_value(trip_count_tn))
			 : 0;
#else
				 ...
				 bool trace)
{
  ...
  if (!remove) {
	...
	BB_Remove_Op(body, op);
      } else if (True_TN != NULL) {
	// If a result of the OP are not used, try replacing
	// the result with True_TN.
	// 14768: On architectures that don't use predicates,
	// True_TN can be NULL.
	for (INT i = 0; i < OP_results(op); i++) {
	  TN *tn = OP_result(op, i);
	  if (TN_is_register(tn) && 
...
void CG_LOOP::Determine_SWP_Unroll_Factor()
{
...
#else
  const bool swp_trace = Get_Trace(TP_SWPIPE, UNROLL_FLAG);
#endif
  ...
  for (i = min_unr; i <= max_unr; i++) {
    if (i * loop_size > loop_size_limit) break;
  ...
  if (SWP_Options.Pow2_Unroll_Times) {
    // Round up unroll_times to nearest power of two.
#if 0
    // __builtin_clz not always available.
    INT new_unroll_times = 1 << (8 * sizeof(UINT) -
				 __builtin_clz((UINT) unroll_times - 1));
#else
    // Code assumes unroll_times < 2^31.
    INT new_unroll_times = unroll_times - 1;
    new_unroll_times |= new_unroll_times >> 16;
    new_unroll_times |= new_unroll_times >>  8;
    new_unroll_times |= new_unroll_times >>  4;
    new_unroll_times |= new_unroll_times >>  2;
    new_unroll_times |= new_unroll_times >>  1;
    new_unroll_times += 1;
#endif
    if (new_unroll_times <= SWP_Options.Max_Unroll_Times) {
      unroll_times = new_unroll_times;
    }
  }
#endif
...
BOOL CG_LOOP_Optimize(LOOP_DESCR *loop, vector<SWP_FIXUP>& fixup)
{
  ...
  switch (action) {
	...
	CG_LOOP_Trace_Loop(loop, "*** Before ebo 2/ after unrolling  ***");
      ...
      // Not doing this EBO causes stream to fail: why does it call
      // EBO_Before_Unrolling here?
      cg_loop.EBO_Before_Unrolling();
	...
	CG_LOOP_Trace_Loop(loop, "*** Before swp / after Ind. Var. Removal  ***");
      ...
      // KEY: fall-through to unrolling if swp fails
      BOOL swp_success = FALSE;
      
      #ifdef TARG_IA64
      if (!(swp_success = Perform_SWP(cg_loop, fixup, true /*doloop*/))) {
      #else
      SWP_DRIVER swp;
      if (!(swp_success = swp.Perform_SWP (cg_loop, SWP_Do_Loop))) {
      #endif
        Undo_SWP_Branch(cg_loop, true /*is_doloop*/);
	CG_LOOP_Remove_Notations(loop, CG_LOOP_prolog, CG_LOOP_epilog);
	cg_loop.Recompute_Liveness();
      }
      // Fall through if SWP failed and if we never unrolled above
      if (swp_success || cg_loop.Unroll_factor() != 1)
        break;
    }
...
#ifdef KEY
      // Determine_Post_Ntimes must run before Build_CG_LOOP_Info.
      cg_loop.Determine_Post_Ntimes();
#endif
      cg_loop.Build_CG_LOOP_Info(TRUE);
	...
	CG_LOOP_Trace_Loop(loop, "*** Before SINGLE_BB_DOLOOP_UNROLL ***");
      ...
      Unroll_Make_Post_Remainder_Loop(cg_loop);
      // Unroll_Make_Post_Remainder_Loop changes the BB set involved in
    ...
    case SINGLE_BB_WHILELOOP_SWP:
    {
      ...
      #ifndef TARG_IA64
      Fail_FmtAssertion ("Should not reach while loop SWP");
      #endif
	...
	Fix_Backpatches(cg_loop, trace_loop_opt);
        ...
        #ifdef TARG_IA64
        if (!Perform_SWP (cg_loop, fixup, false /*whileloop*/)) {
        #else
	SWP_DRIVER swp;
	if (!swp.Perform_SWP (cg_loop, SWP_While_Loop)) {
        #endif  
