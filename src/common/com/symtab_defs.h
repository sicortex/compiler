...
enum ST_FLAGS_EXT
{
    ...
    ST_INITV_IN_OTHER_ST   =   0x04,	// ST is being used as an initianliation
					// offset by other symbol
    ST_IS_PURE_VFUNC    = 0x40,         // ST is pure virtual function

    ST_IS_THREAD_LOCAL     =   0x08,	// ST is thread-local (GCC __thread)
#endif
        /* (cbr) vague linkage support */
    ST_IS_COMDAT = 0x8000,        //  C++ PU need will be generated by any
                                      // translation unit than needs it
    ST_HAS_ALIAS = 0x10000,        // symbol is base of aliases
    ST_ALIAS_USED= 0x20000,        // symbol is an alias and is referenced
    ST_IS_USED =   0x40000,        // symbol has used attribute
    ST_IS_NOT_SIZED =   0x80000,       // symbol has not its size for the moment
}; // ST_FLAGS_EXT
...
#define PU_NEED_TRAMPOLINE	0x0000020000000000LL // a nested function whose address is taken
