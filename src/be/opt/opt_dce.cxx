		   ...
		   BOOL rgn_exit ) const
{
  ...
  if ( bb->Next() != goto_bb ) {
    ...
    if ( Tracing() ) {
      fprintf ( TFile, "<DCE> Add statement to BB:%" PRIdPTR ":\n", bb->Id() );
...
{
  if ( Tracing() ) {
    fprintf( TFile, "DCE::Replace_condbr_with_uncondbr: Updating bb:%" PRIdPTR ";\n",
	     bb->Id() );
    fprintf( TFile, "                      new sole successor is bb:%" PRIdPTR ".\n",
	      goto_bb->Id() );
    fflush( TFile );
  }
  ...
  if ( Tracing() ) {
    fprintf( TFile, "DCE::Replace_condbr_with_uncondbr: Done with bb:%" PRIdPTR "\n",
	     bb->Id() );
    fflush( TFile );
  }
}
...
DCE::Check_constant_cond_br( BB_NODE *bb ) const
{
  ...
  if ( Tracing() ) {
    fprintf( TFile, "DCE::Remove_br in bb:%" PRIdPTR " (%p)\n",bb->Id(),bb);
  }
...
DCE::Replace_condition_with_constant( BB_NODE *bb, INT64 val ) const
{
  ...
  if ( Tracing() ) {
    fprintf( TFile, "Replaced bb:%" PRIdPTR " condition with %" SCNd64 "\n",
	     bb->Id(), val );
  }
}
				 ...
				 BB_NODE_SET *path ) const
{
  ...
  FOR_ALL_ELEM( cdbb, rcfg_iter, Init(path) ) {
	...
	break;
      ...
      fprintf( TFile, "origbb:%" PRIdPTR " path: ", origbb->Id() );
      path->Print(TFile);
      fprintf( TFile, "\n" );
      FmtAssert( FALSE,
  ...
  FOR_ALL_ELEM(bb, cfg_iter, Init()) {
    ...
    for (vector<BB_NODE *>::const_iterator current_iter = back_blocks.begin();
	 current_iter != back_blocks.end(); ++current_iter) {
      ...
      if (Tracing()) fprintf(TFile," %" PRIdPTR "", current_bb->Id());
...
DCE::Update_predecessor_lists( BB_NODE *bb ) const
{
  ...
  if ( Tracing() ) {
    fprintf( TFile, "DCE::Update_predecessor_lists: Updating bb:%" PRIdPTR "\n",
	     bb->Id() );
    fflush( TFile );
  }
  ...
  if ( Tracing() ) {
    fprintf( TFile, "DCE::Update_predecessor_lists: Done with bb:%" PRIdPTR "\n",
	     bb->Id() );
...
DCE::Check_for_label( BB_NODE *bb ) const
{
  ...
  if ( label_stmt == NULL ) {
    ...
    if ( Tracing() )
      fprintf( TFile, "DCE::Check_for_label: Add label to BB:%" PRIdPTR "\n", bb->Id() );
  }
    ...
    if (bb1 != NULL) {
      ...
      for (stmt = bb->First_stmtrep(); stmt; stmt = next_stmt) {
	    ...
	    stmt->Opr() == OPR_EXC_SCOPE_END) {
          if (Tracing()) {
	    fprintf(TFile, "Moving OPR_EXC_SCOPE_%s from bb:%" PRIdPTR " to bb:%" PRIdPTR "\n",
...
DCE::Unreachable_code_elim( void ) const
{
  ...
  for ( bb = _cfg->First_bb(); bb != NULL; bb = nextbb ) {
	...
	Check_for_label( bb );
      ...
      if ( Tracing() ) {
	fprintf( TFile, "DCE: Removing stmts in BB%" PRIdPTR " (0x%p)\n", 
		 bb->Id(), bb );
      }
...
#endif
	if ( Tracing() ) {
	  fprintf( TFile, "DCE: Removed BB%" PRIdPTR "\n",bb->Id());
	  fflush( TFile );
	}
      }
    }
...
#if defined(KEY) // deleting fetch of MTYPE_M return value can cause lowerer to omit
...
#if defined(KEY) // bugs 5401 and 5267
  if (OPERATOR_is_scalar_store(opr) && 
...
DCE::Add_path_to_ipdom( BB_NODE *bb ) const
{
  ...
  if ( Tracing() ) {
    fprintf( TFile, "DCE::Add_path_to_ipdom: add bb%" PRIdPTR " -> ipdom bb%" PRIdPTR "\n",
      bb->Id(), ipdom->Id() );
  }
...
DCE::Update_region_information( void ) const
{
  ...
  FOR_ALL_ELEM( regbb, reg_iter, Init( Region_start_bbs() ) ) {
	...
	if ( Tracing() ) {
	  fprintf( TFile, 
		   "Update_region_information: removed region bb%" PRIdPTR "->bb%" PRIdPTR "\n",
    ...
    FOR_ALL_ELEM( pred, pred_iter, Init( bb->Pred() ) ) {
      ...
      if (branch_stmt != NULL) {
        ...
        if ((opr == OPR_GOTO ||
          ...
          if (Tracing())
            fprintf( TFile, "  changing label %u to %" SCNd32 " in BB%" PRIdPTR "\n",
...
DCE::Update_branch_to_bb_labels( BB_NODE *bb ) const
{
  ...
  if (Tracing())
    fprintf( TFile, "DCE::Update_branch_to_bb_labels for BB%" PRIdPTR "\n",
	     bb->Id());
...
DCE::Remove_dead_statements( void )
{
  ...
  for ( bb = _cfg->First_bb(); bb != NULL; bb = nextbb ) {
	...
	for (stmt = bb->First_stmtrep(); stmt != NULL; stmt = nextstmt){
	  nextstmt = stmt->Next();
	  if ( ! stmt->Live_stmt() ) {
	    fprintf( TFile, "Remove dead statement from live BB:%" PRIdPTR "\n",
		     bb->Id() );
	    stmt->Print( TFile );
	  }
	}
	...
	bb->Set_reached();
      ...
      if ( Tracing() ) {
	fprintf( TFile, "DCE_A: BB%" PRIdPTR " unreached (0x%p)\n", bb->Id(), bb );
      }
	...
	if ( Tracing() ) {
	  fprintf( TFile, "DCE_A: Removed BB%" PRIdPTR " (%p)\n", bb->Id(), bb );
	}
      }
      else {
	bb->Set_reached();
...
#if defined( KEY) // needed due to fix for bug 8690
