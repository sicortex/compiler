	          ...
	          ALIAS_MANAGER *alias_mgr)
{
  ...
  if (phase == MAINOPT_PHASE) {
#if 0
    if (OPT_Reassoc_For_Cse && ! dont_opt)
      WN_reassoc_deepest_blocks(wn_orig);
#endif
    ...
    if (OPT_Reassoc_For_Cse)
      WN_reassoc_deepest_blocks(wn_tree);
#endif
  } // if (phase == MAINOPT_PHASE)
  else {
    if ( Get_Trace( TP_GLOBOPT, OPT_LOWER_FLAG ) ) {
...
#ifdef KEY // copy propagation can enable more opportunities
        comp_unit->Do_bitwise_dce(FALSE /* copy propatage off */);
#endif
      }
    ...
    if (num_fields_accessed == 1)
    {
          ...
          TY_size(complete_struct_relayout_candidate_ty_idx[i]) < 128)
      {
        if (Get_Trace(TP_IPL, 1))
          fprintf(TFile, "ipl -> complete_struct_relayout_candidate = %" SCNd64 " %s\n",
