...
if /usr/lib64/ccache/bin/gcc -E -xc -traditional -P "$@" - < $dir/$file > tmp.options.cpp ; then
    if awk -f $dir/sort_options.awk tmp.options.cpp ; then
        # note that some linux versions of sort are broken,
        # so we actually re-sort within table for safety.
        if sort -r tmp.options > tmp.options.sort ; then
            echo "%%% OPTIONS"
