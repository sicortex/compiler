				...
				BOOL                trace)
{
  ...
  if (trace) {
    fprintf(TFile, "Pending(%" PRIdPTR ") += %" PRIdPTR "\n", Id(), item->Id());
  }
				  ...
				  const BOOL                trace)
{
  ...
  if (trace) {
    fprintf(TFile, "Pending(%" PRIdPTR ") += Pending(%" PRIdPTR ")\n", Id(), that.Id());
  }
			 ...
			 const BOOL                trace)
{
  if (this != &that) {
    ...
    if (trace) {
      fprintf(TFile, "%" PRIdPTR " <-- Join(%" PRIdPTR ",%" PRIdPTR ")\n",
			  ...
			  IP_ALIAS_CLASS_REP *global_class) const
{
  ...
  case IP_ACR_BOTTOM_TYPE:
    ...
    break;
  case IP_ACR_REF_TYPE:
    fprintf(fp, " --> (%" PRIdPTR "<%p> x %" PRIdPTR "<%p>)\n",
...
#else
  DevWarn("Recycled %u of %" PRIdPTR " ACR's",
	  IP_ALIAS_CLASS_REP::_recycled_acr_nodes,
	  IP_ALIAS_CLASS_REP::_last_id_used);
#endif
