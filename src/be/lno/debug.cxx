...
{ 
  ...
  if (WN_operator(wn) == OPR_INTCONST) {
    fprintf(stdout, "%" SCNd64 " ", WN_const_val(wn)); 
  } else { 
...
static void Load_mINT64(mINT64* value) 
{
  load_buffer(); 
  scan_blanks_and_tabs(); 
  sscanf(buffer + buffer_start, "%" SCNd64 "", value); 
  skip_to_separator(SKIP_NUMERIC); 
}
		       ...
		       INT cc)
{
  ...
  switch(opr) {
   case OPR_CONST:
    ...
    break;
   case OPR_INTCONST:
    cc += sprintf(buffer + cc, "%" SCNd64 "", WN_const_val(wn));
    break;
   case OPR_LDID:
				   ...
				   const NODE_ID id_array[])
{
  ...
  for (INT i = 0; i < n_ids; ++i) {
    VINDEX16 v = VINDEX16(uintptr_t(id_array[i]));
    WN *wn = dg->Get_Wn(v);
    fprintf(stdout, "V ");
    this_node(wn);
    fprintf(stdout, "V#%d ", v);
...
void DV_DEPS_CALLBACK::Edge_Select(const EDGE_ID& edge_id) 
{
  ...
  VINDEX16 v_src = VINDEX16(uintptr_t(edge_id.src));
  VINDEX16 v_dst = VINDEX16(uintptr_t(edge_id.dst));
  EINDEX16 e;
...
static void WB_symbol()
{
  if (buffer[buffer_start] == '<') { 
    ...
    fprintf(stdout, "ST_IDX: %" PRIuPTR "\n", st_idx);
    Print_ST(stdout, st, TRUE);
  } else if (isdigit(buffer[buffer_start])) { 
    ST_IDX st_idx;  
    (void) sscanf(buffer + buffer_start, "%" PRIuPTR , &st_idx);
    skip_to_separator(SKIP_NUMERIC);
    ST* st = &St_Table[st_idx]; 
    fprintf(stdout, "ST_IDX: %" PRIuPTR "\n", st_idx);
    Print_ST(stdout, st, TRUE);
...
static void WB_type()
{
  if (buffer[buffer_start] == '<') {
    ...
    TY_IDX ty_idx = 0; 
    (void) sscanf(buffer + buffer_start, "%" SCNd64 , &ty_idx);
    skip_to_separator(SKIP_NUMERIC);
  ...
  case OPC_DO_LOOP:
    {
	  ...
	  fprintf(fp, " (EXACT %" SCNd64 ")", iteration_count); 
	  ...
	  upper_bound = dli->Est_Max_Iterations_Index;  
        if (upper_bound != -1LL)   
	  fprintf(fp, " (UPPER_BOUND %" SCNd64 ")", upper_bound);
        if (dli->Num_Iterations_Profile)
	  fprintf(fp, " (FEEDBACK %" SCNd64 ")", dli->Est_Num_Iterations);
        else if (iteration_count == -1LL && dli->Est_Num_Iterations != -1LL) 
	  fprintf(fp, " (ESTIMATE %" SCNd64 ")", dli->Est_Num_Iterations); 
...
static void WBT_FORMULA_EVALUATE()
{
  ...
  for (INT i = 0; i < count; i++) {
    ...
    case 2: 
      ...
      fprintf(stdout, "Value is %" SCNd64 "\n", vars_int64[i]);   
      break; 
    } 
  }
}
