...
// ====================================================================
// The following conversions are used to support 128-bit IEEE quad float
// on hosts (like x86) which do not support 128-bit quad.  They are not
// polished; they might not handle NAN properly, truncating/rounding
// might not be quite right, and big endian is not implemented.
// ====================================================================

static long double
F16_To_RLD(F16_TYPE q)
{
  // Convert F16_TYPE to 128-bit host long double
...
static F16_TYPE
RLD_To_F16(long double ld)
{
  // Convert 128-bit host long double to F16_TYPE
  // Be careful of alignment.
  union {
    long double val;
    F16_TYPE f16val;
...
#if 0 /*def HOST_SUPPORTS_F16_LONG_DOUBLE */

static F16_TYPE F8_To_F16(double d) { return RLD_To_F16((long double) d); }
static F16_TYPE F4_To_F16(float s)  { return RLD_To_F16((long double) s); }
static double F16_To_F8(F16_TYPE q) { return (double) F16_To_RLD(q); }
static float F16_To_F4(F16_TYPE q)  { return (float) F16_To_RLD(q); }
static F16_TYPE I8_To_F16(INT64 x)  { return RLD_To_F16((long double) x); }
static F16_TYPE U8_To_F16(UINT64 x) { return RLD_To_F16((long double) x); }
static INT64 F16_To_I8(F16_TYPE q)  { return (INT64) F16_To_RLD(q); }
static UINT64 F16_To_U8(F16_TYPE q) { return (UINT64) F16_To_RLD(q); }
...
static F16_TYPE
F10_To_F16(F16_TYPE q)
{
  // Convert 80-bit long double into 128-bit quad for cross compiler workaround
  q.qval[3]  = q.qval[2] << 16;
  q.qval[3] |= (q.qval[1] >> 15) & 0x0000ffff;
  q.qval[2]  = (q.qval[1] << 17) & 0xfffe0000;
  q.qval[2] |= (q.qval[0] >> 15) & 0x0001ffff;
  q.qval[1]  = (q.qval[0] << 17) & 0xfffe0000;
  q.qval[0]  = 0;
  return q;
}

static F16_TYPE
F16_To_F10(F16_TYPE q)
{
  // Convert 128-bit quad host 80-bit long double for trace printing
  q.qval[0]  = (q.qval[1] >> 17) & 0x00007fff;
  q.qval[0] |= (q.qval[2] << 15) & 0xffff8000;
  q.qval[1]  = (q.qval[2] >> 17) & 0x00007fff;
  q.qval[1] |= (q.qval[3] << 15) & 0x7fff8000;
  q.qval[2] = q.qval[3] >> 16;
  INT32 expon = q.qval[2] & 0x00007fff;
  if (expon != 0 && expon != 32767) q.qval[1] |= 0x80000000;
  return q;
}
...
F8_To_F16(double d)
{
  // Convert 64-bit double to 128-bit quad host
  union {
    double val;
    INT32 dval[2];
  } dr;
  dr.val = d;
  F16_TYPE q;
  // sign extended exponent and mantissa
  // Change expon offset from 2^11-1 to 2^15-1
  q.qval[3]  = (((dr.dval[1] << 1) >> 5) & 0x7fffffff) ^ 0x3c000000;
  q.qval[3] |= dr.dval[1] & 0x80000000;  // sign
  q.qval[2]  = (dr.dval[1] << 28) & 0xf0000000; // mantissa
  q.qval[2] |= (dr.dval[0] >>  4) & 0x0fffffff;
  q.qval[1]  = (dr.dval[0] << 28) & 0xf0000000;
  q.qval[0]  = 0x00000000;
  // printf("double %8f %8x %8x\n", d, dr.dval[1], dr.dval[0]);
  // printf("quad           %8x %8x %8x %8x\n",
  //	 q.qval[3], q.qval[2], q.qval[1], q.qval[0]);
  return q;
}
...
F4_To_F16(float s)
{
  // Convert 32-bit single to 128-bit quad host
  union {
    float val;
    INT32 sval;
  } sr;
  ...
  F16_TYPE q;
  // sign extended exponent and mantissa
  // Change expon offset from 2^8-1 to 2^15-1
  q.qval[3]  = (((sr.sval << 1) >> 8) & 0x7fffffff) ^ 0x3f800000;
  q.qval[3] |= sr.sval & 0x80000000;  // sign
  q.qval[2]  = (sr.sval << 25) & 0xfe000000;
  q.qval[1]  = 0x00000000;
  q.qval[0]  = 0x00000000;
  return q;
}
...
static double
F16_To_F8(F16_TYPE q)
{
  // Convert 128-bit quad to 64-bit double
  union {
    double val;
    INT32 dval[2];
  } d;
  d.dval[1]  = q.qval[3] & 0x80000000;  // sign
  // exponent and mantissa; Change expon offset from 2^15-1 to 2^11-1
  d.dval[1] |= ((q.qval[3] <<  4) & 0x7ffffff0) ^ 0x40000000;
  d.dval[1] |= (q.qval[2] >> 28) & 0x0000000f;  // mantissa
  d.dval[0]  = (q.qval[2] <<  4) & 0xfffffff0;
  d.dval[0] |= (q.qval[1] >> 28) & 0x0000000f;
  return d.val;
}

static float
F16_To_F4(F16_TYPE q)
{
  // Convert 128-bit quad to 32-bit single
  union {
    float val;
    INT32 sval;
  } s;
  s.sval  = q.qval[3] & 0x80000000;  // sign
  // exponent and mantissa; Change expon offset from 2^15-1 to 2^8-1
  s.sval |= ((q.qval[3] <<  7) & 0x7fffff80) ^ 0x40000000;
  s.sval |= (q.qval[2] >> 25) & 0x0000007f;  // mantissa
  return s.val;
}
...
U8_To_F16(UINT64 x)
{
  // Convert 64-bit unsigned integer to 128-bit quad host
  F16_TYPE q;
  if (x == 0) return F4_To_F16(0.0);
  INT32 expon = 63 + 0x00003fff;
  INT64 sx = (INT64) x;
  while (sx > 0) {
    expon -= 1;
    sx = sx << 1;
  }
  q.qval[3]  = (expon << 16) & 0x7fff0000;  // exponent and sign bit 0
  q.qval[3] |= (sx >> 47) & 0x0000ffff;  // mantissa, chop first 1 bit
  q.qval[2]  = (INT32) (sx >> 15);  // mantissa
  q.qval[1]  = (INT32) (sx << 17) & 0xfffe0000;
  q.qval[0]  = 0x00000000;
  return q;
}
...
I8_To_F16(INT64 sx)
{
  // Convert 64-bit signed integer to 128-bit quad host
  // printf("I8_To_F16 %" SCNd64 "\n", sx);
  F16_TYPE q;
  if (sx == 0) return F4_To_F16(0.0);
  INT32 sign = (sx < 0);
  if (sign) sx = -sx;
  INT32 expon = 63 + 0x00003fff;
  while (sx > 0) {
  ...
  // printf("expon %d %d\n", expon, expon - 0x00003fff);
  q.qval[3]  = (expon << 16) & 0x7fff0000;  // exponent
  q.qval[3] |= sign << 31;  // sign bit
  q.qval[3] |= (sx >> 47) & 0x0000ffff;  // mantissa, chop first 1 bit
  q.qval[2]  = (INT32) (sx >> 15);  // mantissa
  q.qval[1]  = (INT32) (sx << 17) & 0xfffe0000;
  q.qval[0]  = 0x00000000;
...
static UINT64
F16_To_U8(F16_TYPE q)
{
  // Convert 128-bit quad to 64-bit unsigned integer
  // printf("quad           %8x %8x %8x %8x\n",
  //	 q.qval[3], q.qval[2], q.qval[1], q.qval[0]);
  INT32 sign = q.qval[3] & 0x80000000;
  INT32 expon = ((q.qval[3] >> 16) & 0x00007fff) - 0x00003fff;
  // printf("exponent %d\n", expon);
  if (expon < 0) return 0;

  UINT64 x = 1;  // Insert leading bit of mantissa
  x = (x << 16) | (q.qval[3] & 0x0000ffff);
  x = (x << 32) | q.qval[2];
  x = (x << 15) | ((q.qval[1] >> 17) & 0x0001ffff);
...
static INT64
F16_To_I8(F16_TYPE q)
{
  return (INT64) F16_To_U8(q);
}

#else  // LITTLE_ENDIAN

static F16_TYPE F8_To_F16(double d)
{
  FmtAssert(0, ("F8_To_F16: Big endian not supported"));
}
static F16_TYPE F4_To_F16(float s)
{
  FmtAssert(0, ("F4_To_F16: Big endian not supported"));
}
static double F16_To_F8(F16_TYPE q)
{
  FmtAssert(0, ("F16_To_F8: Big endian not supported"));
}
static float F16_To_F4(F16_TYPE q)
{
  FmtAssert(0, ("F16_To_F4: Big endian not supported"));
}
static F16_TYPE I8_To_F16(INT64 x)
{
  FmtAssert(0, ("I8_To_F16: Big endian not supported"));
}
static F16_TYPE U8_To_F16(UINT64 x)
{
  FmtAssert(0, ("U8_To_F16: Big endian not supported"));
}
static INT64 F16_To_I8(F16_TYPE q)
{
  FmtAssert(0, ("F16_To_I8: Big endian not supported"));
}
static UINT64 F16_To_U8(F16_TYPE q)
{
  FmtAssert(0, ("F16_To_U8: Big endian not supported"));
}
...
static TCON complex_sqrt(TCON c0)
{
   ...
   switch (TCON_ty(c0)) {
    ...
    case MTYPE_C16:
      {
	long double qr, qi, qmag, rqr, rqi, q0, q05;
	qr = F16_To_RLD(TCON_F16(c0));
	qi = F16_To_RLD(TCON_IF16(c0));
	q0 = 0.0;
	q05 = 0.5;
	...
	TCON_F16(r) = RLD_To_F16(rqr);
	TCON_IF16(r) =  RLD_To_F16(rqi);
      }
      break;
#endif
...
static TCON complex_divide(TCON c0, TCON c1)
{
   ...
   switch (TCON_ty(c0)) {
    ...
    case MTYPE_C16:
      {
	long double c0r, c0i, c1r, c1i, t1, t2, t3, t4, ar, ai, q0;
	INT err;
	q0 = 0.0;
	c0r = F16_To_RLD(TCON_F16(c0));
	c0i = F16_To_RLD(TCON_IF16(c0));
	c1r = F16_To_RLD(TCON_F16(c1));
	c1i = F16_To_RLD(TCON_IF16(c1));
	ar = c1r;
	ai = c1i;
	if (ar < q0) ar = -ar;
	if (ai < q0) ai = -ai;
	...
	TCON_F16(r) = RLD_To_F16(t3);
	TCON_IF16(r) = RLD_To_F16(t1);
      }
      break;
#endif
...
#undef DEBUG_FOLD
#ifdef DEBUG_FOLD
   printf("Folding %s on 0x%" SCNx64 " (%s), 0x%" SCNx64 " (%s), result",OPCODE_name(op),
   ...
   *folded = TRUE;
   ...
   if (DEBUG_Trap_Uv && MTYPE_float(optype) && MTYPE_float(desc) &&
     ...
     case MTYPE_F16:
#if 0 /* def HOST_SUPPORTS_F16_LONG_DOUBLE */
       ld0 = F16_To_RLD(TCON_F16(c0));
       ld1 = F16_To_RLD(TCON_F16(c1));
       switch (opr) {
       case OPR_EQ: TCON_v0(c0) = (ld0 == ld1); break;
       case OPR_NE: TCON_v0(c0) = (ld0 != ld1); break;
       case OPR_LT: TCON_v0(c0) = (ld0 <  ld1); break;
       case OPR_LE: TCON_v0(c0) = (ld0 <= ld1); break;
       case OPR_GT: TCON_v0(c0) = (ld0 >  ld1); break;
       case OPR_GE: TCON_v0(c0) = (ld0 >= ld1); break;
       }
       TCON_v1(c0) = 0;
       TCON_v2(c0) = 0;
       TCON_v3(c0) = 0;
     ...
     case MTYPE_C16:
       ...
       ld0 = F16_To_RLD(TCON_IF16(c0));
       ld1 = F16_To_RLD(TCON_IF16(c1));
       switch (opr) {
       case OPR_EQ: TCON_v0(c0) &= (ld0 == ld1); break;
       case OPR_NE: TCON_v0(c0) |= (ld0 != ld1); break;
       }
    ...
    */
    ...
    case OPC_F16F4CVT:
      Is_True(TCON_ty(c0) == MTYPE_F4, ("Illegal operand to %s", OPCODE_name(op)));
      c0 = Targ_Conv(MTYPE_F16, c0);
      break;
    case OPC_F4F16CVT:
      Is_True(TCON_ty(c0) == MTYPE_F16, ("Illegal operand to %s", OPCODE_name(op)));
      c0 = Targ_Conv(MTYPE_F4, c0);
      break;
    case OPC_F16F8CVT:
      Is_True(TCON_ty(c0) == MTYPE_F8, ("Illegal operand to %s", OPCODE_name(op)));
    ...
    case OPC_I4F16RND:
#if 0 /* def HOST_SUPPORTS_F16_LONG_DOUBLE */
      if (F16_To_RLD(TCON_F16(c0)) >= 0.0) {
	TCON_F16(c0) = RLD_To_F16(F16_To_RLD(TCON_F16(c0)) + 0.5);
      } else {
	TCON_F16(c0) = RLD_To_F16(F16_To_RLD(TCON_F16(c0)) - 0.5);     
      }
      c0 = Targ_Conv(MTYPE_I4,c0);
...
#if 0 /* def HOST_SUPPORTS_F16_LONG_DOUBLE */
      t1 = Targ_Conv(MTYPE_I4, c0);
      t2 = Targ_Conv(MTYPE_F16,t1);
      if (F16_To_RLD(TCON_F16(t2)) > F16_To_RLD(TCON_F16(c0))) {
	 TCON_I4(t1) -= 1;
      }
      c0 = t1;
#else
    ...
    case OPC_F16ABS:
#if 0 /* def HOST_SUPPORTS_F16_LONG_DOUBLE */
      if (F16_To_RLD(TCON_F16(c0)) < 0) {
	TCON_F16(c0) = RLD_To_F16(- F16_To_RLD(TCON_F16(c0)));
      }
#else
      *folded = FALSE;
#endif
    ...
    case OPC_F16NEG:
      ...
      TCON_F16(c0) = RLD_To_F16(- F16_To_RLD(TCON_F16(c0)));
#else
      // Simplifier crashes if we do "*folded = false" here
      TCON_v3(c0) ^= 0x80000000;
#endif
      break;
    case OPC_C4NEG:
      TCON_R4(c0)  = -TCON_R4(c0);
...
#if 0 /* def HOST_SUPPORTS_F16_LONG_DOUBLE */
      TCON_F16(c0) = RLD_To_F16(- F16_To_RLD(TCON_F16(c0)));
      TCON_IF16(c0) = RLD_To_F16(- F16_To_RLD(TCON_IF16(c0)));
#else
      // Simplifier crashes if we do "*folded = false" here
      TCON_v3(c0) ^= 0x80000000;
      TCON_iv3(c0) ^= 0x80000000;
#endif
...
#if 0 /* def HOST_SUPPORTS_F16_LONG_DOUBLE */
      if (F16_To_RLD(TCON_F16(c0)) >= 0.0) {
	TCON_F16(c0) = RLD_To_F16(sqrtl(F16_To_RLD(TCON_F16(c0))));
      } else {
	*folded = FALSE;
      }
#else
   ...
   case OPC_C16SQRT:
...
#if 0 /* def HOST_SUPPORTS_F16_LONG_DOUBLE */
      {
	long double a, b, c, d, r1, r2, r3;
	a = F16_To_RLD(TCON_F16(c0));
	b = F16_To_RLD(TCON_IF16(c0));
	c = F16_To_RLD(TCON_F16(c1));
	d = F16_To_RLD(TCON_IF16(c1));
	r1 = a * c;
	r2 = b * d;
	r3 = r1 - r2;
	r1 = a * d;
	r2 = b * c;
	r1 = r1 + r2;
	TCON_F16(c0) = RLD_To_F16(r3);
	TCON_IF16(c0) = RLD_To_F16(r1);
      }
#else
      *folded = FALSE;
#endif
...
/*	 ErrMsg( EC_Ill_UModulus );*/
	*folded = FALSE;
    ...
    case OPC_FQRECIP:
      ...
      break;
    case OPC_F16RECIP:
      c0 = Targ_WhirlOp(OPC_F16DIV,Host_To_Targ_Float(MTYPE_F16,1.0),c0,folded);
      break;
    case OPC_C4RECIP:
    ...
    case OPC_CQRECIP:
      ...
      break;
    case OPC_C16RECIP:
      c0 = Targ_WhirlOp(OPC_C16DIV,Host_To_Targ_Float(MTYPE_C16,1.0),c0,folded);
      break;
    ...
    case OPC_FQRSQRT:
      ...
      break;
    case OPC_F16RSQRT:
      c0 = Targ_WhirlOp(OPC_F16SQRT,c0,c0,folded);
      if (*folded) c0 = Targ_WhirlOp(OPC_F16DIV,Host_To_Targ_Float(MTYPE_F16,1.0),c0,folded);
      break;
    case OPC_C4RSQRT:
    ...
    case OPC_CQRSQRT:
      ...
      break;
    case OPC_C16RSQRT:
      c0 = Targ_WhirlOp(OPC_C16SQRT,c0,c0,folded);
      if (*folded) c0 = Targ_WhirlOp(OPC_C16DIV,Host_To_Targ_Float(MTYPE_C16,1.0),c0,folded);
      break;
...
#if 0 /* def HOST_SUPPORTS_F16_LONG_DOUBLE */
    case OPC_F16MIN:
      TCON_F16(c0) = (F16_To_RLD(TCON_F16(c0)) < F16_To_RLD(TCON_F16(c1)) ?
		      TCON_F16(c0) : TCON_F16(c1));
      break;
    case OPC_F16MAX:
      TCON_F16(c0) = (F16_To_RLD(TCON_F16(c0)) < F16_To_RLD(TCON_F16(c1)) ?
		      TCON_F16(c1) : TCON_F16(c0));
      break;
#else
    case OPC_F16MIN:
    case OPC_F16MAX:
    ...
    case OPC_CQCOMPLEX:
      ...
      break;
    case OPC_C16COMPLEX:
      TCON_IF16(c0) = TCON_F16(c1);
      TCON_ty(c0) = MTYPE_C16;
      break;
    ...
    case OPC_FQREALPART:
      TCON_ty(c0) = MTYPE_FQ;
      break;
    case OPC_F16REALPART:
      TCON_ty(c0) = MTYPE_F16;
      break;
    ...
    case OPC_FQIMAGPART:
      ...
      TCON_ty(c0) = MTYPE_FQ;
      break;
    case OPC_F16IMAGPART:
      TCON_F16(c0) = TCON_IF16(c0);
      TCON_ty(c0) = MTYPE_F16;
      break;
...
#define FROM_TO(type_from, type_to) (type_from)*(MTYPE_LAST+1)+(type_to)
  ...
  */
  switch ( FROM_TO(ty_from, ty_to) ) {
    ...
    case FROM_TO(MTYPE_C8, MTYPE_C16):
      TCON_F16(r) = F8_To_F16(TCON_R8(c));
      TCON_IF16(r) = F8_To_F16(TCON_IR8(c));
      break;
    case FROM_TO(MTYPE_C4, MTYPE_C16):
      TCON_F16(r) = F4_To_F16(TCON_R4(c));
      TCON_IF16(r) = F4_To_F16(TCON_IR4(c));
      break;
    case FROM_TO(MTYPE_F16, MTYPE_C16):
      TCON_F16(r) = TCON_F16(c);
      TCON_IF16(r) = F4_To_F16(0.0);
      break;
    case FROM_TO(MTYPE_F8, MTYPE_C16):
      TCON_F16(r) = F8_To_F16(TCON_R8(c));
      TCON_IF16(r) = F4_To_F16(0.0);
      break;
    case FROM_TO(MTYPE_F4, MTYPE_C16):
      TCON_F16(r) = F4_To_F16(TCON_R4(c));
      TCON_IF16(r) = F4_To_F16(0.0);
      break;
    case FROM_TO(MTYPE_I8, MTYPE_C16): 
      TCON_F16(r) = I8_To_F16(TCON_I8(c));
      TCON_IF16(r) = F4_To_F16(0.0);
      break;
    case FROM_TO(MTYPE_I4, MTYPE_C16): 
    case FROM_TO(MTYPE_I2, MTYPE_C16): 
    case FROM_TO(MTYPE_I1, MTYPE_C16): 
      TCON_F16(r) = F8_To_F16((double) TCON_u0(c));
      TCON_IF16(r) = F4_To_F16(0.0);
      break;
    case FROM_TO(MTYPE_U8, MTYPE_C16):
      TCON_F16(r) = U8_To_F16(TCON_U8(c));
      TCON_IF16(r) = F4_To_F16(0.0);
      break;
    case FROM_TO(MTYPE_U4, MTYPE_C16):
    case FROM_TO(MTYPE_U2, MTYPE_C16):
    case FROM_TO(MTYPE_U1, MTYPE_C16):
      TCON_F16(r) = F8_To_F16((double) TCON_u0(c));
      TCON_IF16(r) = F4_To_F16(0.0);
      break;
    ...
    case FROM_TO(MTYPE_CQ, MTYPE_C8):
      ...
      break;
#endif /* TARG_NEEDS_QUAD_OPS */
    case FROM_TO(MTYPE_C16, MTYPE_C8):
      TCON_R8(r) = F16_To_F8(TCON_F16(c));
      TCON_IR8(r) = F16_To_F8(TCON_IF16(c));
      break;
    ...
    case FROM_TO(MTYPE_FQ, MTYPE_C8):
      ...
      break;
#endif /* TARG_NEEDS_QUAD_OPS */
    case FROM_TO(MTYPE_F16, MTYPE_C8):
      TCON_R8(r) = F16_To_F8(TCON_F16(c));
      TCON_IR8(r) = 0.0;
    ...
    case FROM_TO(MTYPE_CQ, MTYPE_C4):
      ...
      break;
#endif /* TARG_NEEDS_QUAD_OPS */
    case FROM_TO(MTYPE_C16, MTYPE_C4):
      TCON_R4(r) = F16_To_F4(TCON_F16(c));
      TCON_IR4(r) = F16_To_F4(TCON_IF16(c));
      break;
    ...
    case FROM_TO(MTYPE_FQ, MTYPE_C4):
      ...
      break;
#endif /* TARG_NEEDS_QUAD_OPS */
    case FROM_TO(MTYPE_F16, MTYPE_C4):
      TCON_R4(r) = F16_To_F4(TCON_F16(c));
      TCON_IR4(r) = 0.0;
      break;
    ...
    case FROM_TO(MTYPE_C16, MTYPE_F16):
      TCON_F16(r) = TCON_F16(c);
      break;
    case FROM_TO(MTYPE_C8, MTYPE_F16):
    case FROM_TO(MTYPE_F8, MTYPE_F16):
      TCON_F16(r) = F8_To_F16(TCON_R8(c));
      break;
    case FROM_TO(MTYPE_C4, MTYPE_F16):
    case FROM_TO(MTYPE_F4, MTYPE_F16):
      TCON_F16(r) = F4_To_F16(TCON_R4(c));
      break;
    case FROM_TO(MTYPE_I8, MTYPE_F16): 
      TCON_F16(r) = I8_To_F16(TCON_I8(c));
      break;
    case FROM_TO(MTYPE_I4, MTYPE_F16): 
    case FROM_TO(MTYPE_I2, MTYPE_F16): 
    case FROM_TO(MTYPE_I1, MTYPE_F16): 
      TCON_F16(r) = F8_To_F16((double) TCON_v0(c));
      break;
    case FROM_TO(MTYPE_U8, MTYPE_F16):
      TCON_F16(r) = U8_To_F16(TCON_U8(c));
      break;
    case FROM_TO(MTYPE_U4, MTYPE_F16):
    case FROM_TO(MTYPE_U2, MTYPE_F16):
    case FROM_TO(MTYPE_U1, MTYPE_F16):
      TCON_F16(r) = F8_To_F16((double) TCON_u0(c));
      break;
    ...
    case FROM_TO(MTYPE_CQ, MTYPE_F8):
      ...
      break;
#endif /* TARG_NEEDS_QUAD_OPS */
    case FROM_TO(MTYPE_C16, MTYPE_F8):
      TCON_R8(r) = F16_To_F8(TCON_F16(c));
      break;
    ...
    case FROM_TO(MTYPE_FQ, MTYPE_F8):
      ...
      break;
#endif /* TARG_NEEDS_QUAD_OPS */
    case FROM_TO(MTYPE_F16, MTYPE_F8):
      TCON_R8(r) = F16_To_F8(TCON_F16(c));
      break;
    ...
    case FROM_TO(MTYPE_CQ, MTYPE_F4):
      ...
      break;
#endif /* TARG_NEEDS_QUAD_OPS */
    case FROM_TO(MTYPE_C16, MTYPE_F4):
    case FROM_TO(MTYPE_F16, MTYPE_F4):
      TCON_R4(r) = F16_To_F4(TCON_F16(c));
      break;
    ...
    case FROM_TO(MTYPE_FQ, MTYPE_I8):
      ...
      break;
#endif /* TARG_NEEDS_QUAD_OPS */
    case FROM_TO(MTYPE_C16, MTYPE_I8):
    case FROM_TO(MTYPE_F16, MTYPE_I8):
      TCON_I8(r) = F16_To_I8(TCON_F16(c));
      break;
    ...
    case FROM_TO(MTYPE_FQ, MTYPE_I1):
      ...
      break;
#endif
    case FROM_TO(MTYPE_C16, MTYPE_I4):
    case FROM_TO(MTYPE_C16, MTYPE_I2):
    case FROM_TO(MTYPE_C16, MTYPE_I1):
    case FROM_TO(MTYPE_F16, MTYPE_I4):
    case FROM_TO(MTYPE_F16, MTYPE_I2):
    case FROM_TO(MTYPE_F16, MTYPE_I1):
      TCON_v0(r) = (INT32) F16_To_F8(TCON_F16(c));
      break;
    ...
    case FROM_TO(MTYPE_FQ, MTYPE_U8):
      ...
      break;
#endif
    case FROM_TO(MTYPE_C16, MTYPE_U8):
    case FROM_TO(MTYPE_F16, MTYPE_U8):
      TCON_U8(r) = F16_To_U8(TCON_F16(c));
      break;
    ...
    case FROM_TO(MTYPE_FQ, MTYPE_U1):
      ...
      break;
#endif
    case FROM_TO(MTYPE_C16, MTYPE_U4):
    case FROM_TO(MTYPE_C16, MTYPE_U2):
    case FROM_TO(MTYPE_C16, MTYPE_U1):
    case FROM_TO(MTYPE_F16, MTYPE_U4):
    case FROM_TO(MTYPE_F16, MTYPE_U2):
    case FROM_TO(MTYPE_F16, MTYPE_U1):
      TCON_u0(r) = (UINT32) F16_To_F8(TCON_F16(c));
      break;
...
#define TPB_SIZE 8
   ...
   case MTYPE_FQ:
    ...
    case MTYPE_F16:
      {
	// Convert 128-bit quad host 80-bit long double for printing
	F16_TYPE q = TCON_F16(c);
#if 1 /*ndef HOST_SUPPORTS_F16_LONG_DOUBLE */
	q = F16_To_F10(q);
#endif
	if (fmt == NULL) fmt = "%#21.20Lg";
	sprintf(r, fmt, F16_To_RLD(q));
	// if (fmt == NULL) fmt = "%#21.20Lg";
	// sprintf(r, fmt, TCON_R16(c));
#if !(defined(FRONT_END_C) || defined(FRONT_END_CPLUSPLUS))
	if (re = strchr(r, 'e'))
   ...
   case MTYPE_C16:
      {
	// Convert 128-bit quad host 80-bit long double for printing
	F16_TYPE q0 = TCON_F16(c);
	F16_TYPE q1 = TCON_F16(c);
#if 1 /* ndef HOST_SUPPORTS_F16_LONG_DOUBLE */
	q0 = F16_To_F10(q0);
	q1 = F16_To_F10(q1);
#endif
	if (fmt == NULL) fmt = "%#21.20Lg, %#21.20Lg";
	sprintf(r, fmt, F16_To_RLD(q0), F16_To_RLD(q1));
#if !(defined(FRONT_END_C) || defined(FRONT_END_CPLUSPLUS))
	if (re = strchr(r, 'e'))
	  *re = 'd';
	if (re = strrchr(r, 'e'))
...
Make_Complex ( TYPE_ID ctype, TCON real, TCON imag )
{
  ...
  case MTYPE_C16:
     Set_TCON_F16(c, TCON_F16(real));
     Set_TCON_IF16(c, TCON_F16(imag));
     break;
...
Extract_Complex_Real(TCON complex)
{
  ...
  case MTYPE_C16:
     TCON_ty(c) = MTYPE_F16;
     Set_TCON_F16(c, TCON_F16(complex));
     return c;
...
Extract_Complex_Imag(TCON complex)
{
  ...
  case MTYPE_C16:
     TCON_ty(c) = MTYPE_F16;
     Set_TCON_F16(c, TCON_IF16(complex));
     return c;
...
TCON
Host_To_Targ_Complex_F16(F16_TYPE real, F16_TYPE imag)
{
  static TCON c;
...
Targ_Is_Integral ( TCON tc, INT64 *iv )
{
  ...
  switch (TCON_ty(tc)) {
    ...
    case MTYPE_F16:
      {
	INT64 k = F16_To_I8(TCON_F16(tc));
	F16_TYPE q = I8_To_F16(k);
#if 0 /* def HOST_SUPPORTS_F16_LONG_DOUBLE */
	if (F16_To_RLD(q) == F16_To_RLD(TCON_F16(tc)))
#else
...
Targ_Is_Power_Of_Two ( TCON t )
{
   ...
   switch (TCON_ty(t)) {
    ...
    case MTYPE_F16:
      exponent = ((TCON_v3(t) >> 16) & 0x00007fff);
      mant = (TCON_v3(t) & 0x0000ffff) | TCON_v0(t) | TCON_v1(t) | TCON_v2(t);
      return mant==0 && exponent != 0 && exponent != 0x7fff;
   }
...
#if 0 /* def HOST_SUPPORTS_F16_LONG_DOUBLE */
   // TODO: 128-bits of accuracy needed here
#define M_PILD  3.141592653589793238462643383279531l
#define DEG_TO_RADLD (M_PILD/180.0l)
#define RAD_TO_DEGLD (180.0l/M_PILD)
#endif
   ...
   switch ((INTRINSIC) intrinsic) {
    ...
    case INTRN_F16I8EXPEXPR:
      c0 = Targ_Power(c[0],c[1],folded,MTYPE_F16);
      break;
    ...
    case INTRN_C16I8EXPEXPR:
      c0 = Targ_Power(c[0],c[1],folded,MTYPE_C16);
      break;
    ...
    case INTRN_F16SIGN:
      c0 = Targ_WhirlOp(OPC_F16ABS,c0,c0,folded);
      t = Targ_WhirlOp(OPC_I4F16LT,c[1],F16_Zero_Tcon,folded);
      if (TCON_I4(t)) c0 = Targ_WhirlOp(OPC_F16NEG,c0,c0,folded);
      break;
    ...
    case INTRN_I2F16IQNINT:
      c0 = Targ_Conv(MTYPE_I2,Targ_WhirlOp(OPC_I4F16RND,c[0],c[0],folded));
      break;

    case INTRN_I4F16IQNINT:
      c0 = Targ_WhirlOp(OPC_I4F16RND,c[0],c[0],folded);
      break;

    case INTRN_I8F16IQNINT:
      c0 = Targ_WhirlOp(OPC_I8F16RND,c[0],c[0],folded);
      break;
    ...
    case INTRN_F16DIM:
      c0 = Targ_WhirlOp(OPC_F16SUB,c0,c[1],folded);
      t = Targ_WhirlOp(OPC_I4F16LT,c0,F16_Zero_Tcon,folded);
      if (TCON_I4(t)) c0 = F16_Zero_Tcon;
      break;
    ...
    case INTRN_F16EXP:
      TCON_F16(c0) = RLD_To_F16(expl(F16_To_RLD(TCON_F16(c0))));
      break;
    case INTRN_F16LOG:
      TCON_F16(c0) = RLD_To_F16(logl(F16_To_RLD(TCON_F16(c0))));
      break;
    case INTRN_F16LOG10:
      TCON_F16(c0) = RLD_To_F16(log10l(F16_To_RLD(TCON_F16(c0))));
      break;
    case INTRN_F16COS:
      TCON_F16(c0) = RLD_To_F16(cosl(F16_To_RLD(TCON_F16(c0))));
      break;
    case INTRN_F16SIN:
      TCON_F16(c0) = RLD_To_F16(sinl(F16_To_RLD(TCON_F16(c0))));
      break;
    case INTRN_F16TAN:
      TCON_F16(c0) = RLD_To_F16(tanl(F16_To_RLD(TCON_F16(c0))));
      break;
    case INTRN_F16COSD:
      TCON_F16(c0) = RLD_To_F16(cosl(DEG_TO_RADLD*F16_To_RLD(TCON_F16(c0))));
      break;
    case INTRN_F16SIND:
      TCON_F16(c0) = RLD_To_F16(sinl(DEG_TO_RADLD*F16_To_RLD(TCON_F16(c0))));
      break;
    case INTRN_F16TAND:
      TCON_F16(c0) = RLD_To_F16(tanl(DEG_TO_RADLD*F16_To_RLD(TCON_F16(c0))));
      break;
    case INTRN_F16COSH:
      TCON_F16(c0) = RLD_To_F16(coshl(F16_To_RLD(TCON_F16(c0))));
      break;
    case INTRN_F16SINH:
      TCON_F16(c0) = RLD_To_F16(sinhl(F16_To_RLD(TCON_F16(c0))));
      break;
    case INTRN_F16TANH:
      TCON_F16(c0) = RLD_To_F16(tanhl(F16_To_RLD(TCON_F16(c0))));
      break;
    case INTRN_F16ACOS:
      TCON_F16(c0) = RLD_To_F16(acosl(F16_To_RLD(TCON_F16(c0))));
      break;
    case INTRN_F16ASIN:
      TCON_F16(c0) = RLD_To_F16(asinl(F16_To_RLD(TCON_F16(c0))));
      break;
    case INTRN_F16ATAN:
      TCON_F16(c0) = RLD_To_F16(atanl(F16_To_RLD(TCON_F16(c0))));
      break;
    case INTRN_F16ACOSD:
      TCON_F16(c0) = RLD_To_F16(RAD_TO_DEGLD*acosl(F16_To_RLD(TCON_F16(c0))));
      break;
    case INTRN_F16ASIND:
      TCON_F16(c0) = RLD_To_F16(RAD_TO_DEGLD*asinl(F16_To_RLD(TCON_F16(c0))));
      break;
    case INTRN_F16ATAND:
      TCON_F16(c0) = RLD_To_F16(RAD_TO_DEGLD*atanl(F16_To_RLD(TCON_F16(c0))));
      break;
    case INTRN_F16ATAN2:
      TCON_F16(c0) = RLD_To_F16(atan2l(F16_To_RLD(TCON_F16(c0)),F16_To_RLD(TCON_F16(c[1]))));
      break;
    case INTRN_F16ATAN2D:
      TCON_F16(c0) = RLD_To_F16(RAD_TO_DEGLD*atan2l(F16_To_RLD(TCON_F16(c0)),F16_To_RLD(TCON_F16(c[1]))));
      break;

    case INTRN_F16CIS:
      TCON_ty(c0) = MTYPE_C16;
      TCON_F16(c0) = RLD_To_F16(cosl(F16_To_RLD(TCON_F16(c[0]))));
      TCON_IF16(c0) = RLD_To_F16(sinl(F16_To_RLD(TCON_F16(c[0]))));
      break;

    case INTRN_F16AINT:
      TCON_F16(c0) = RLD_To_F16(truncl(F16_To_RLD(TCON_F16(c0))));
      break;

    case INTRN_F16ANINT:
      if (F16_To_RLD(TCON_F16(c0)) < 0) {
	 TCON_F16(c0) = RLD_To_F16(truncl(F16_To_RLD(TCON_F16(c0))-0.5l));
      } else {
	 TCON_F16(c0) = RLD_To_F16(truncl(F16_To_RLD(TCON_F16(c0))+0.5l));
      }
      break;
...
#else /* No F16 support in library */
      /* These aren't done because we don't have a convenient library yet */
    case INTRN_F16AINT:
    case INTRN_F16ANINT:
    case INTRN_F16EXP:
