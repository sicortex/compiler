...
#ifdef KEY /* Bug 6845, 14801 */
    /* Pointer: perform pointer assignment */
    else if (ATD_POINTER(cpnt_attr_idx)) {
      new_ir_idx = gen_ir(IR_Tbl_Idx, l_idx, Ptr_Asg_Opr,
        ATD_TYPE_IDX(cpnt_attr_idx), line, col, IR_Tbl_Idx, r_idx);
      need_semantics = TRUE;
    }
#endif /* KEY Bug 6845, 14801 */
      ...
      if (active_forall_sh_idx) {
         ...
         if (IR_OPR(ir_idx) != Call_Opr) {
            ...
            if (forall_dependence) {
               ...
               copy_subtree(&forall_tmp_opnd, &forall_tmp_opnd, 0, 0);
...
#ifdef KEY /* Bug 5091 */
	 int idx = OPND_IDX(r_opnd);
	 /* F2003 C537: forbid use-associated non-pointer protected on RHS
	  * of => */
         if ((!ATD_POINTER(idx)) && readonly_protected(idx)) {
            PRINTMSG(OPND_LINE_NUM(r_opnd), 418, Error, OPND_COL_NUM(r_opnd));
            ok = FALSE;
	 }
#endif /* KEY Bug 5091 */
...
#ifdef KEY /* Bug 5091 */
	 /* F2003 C537: forbid use-associated protected (indicated by
	  * "constant" here) nonpointer on RHS of =>. */
         else if ((!exp_desc_r.pointer) && exp_desc_r.constant) {
	    find_opnd_line_and_column(&r_opnd, &opnd_line, &opnd_col);
      ...
      if (ok) {
         ...
         if (active_forall_sh_idx) {
            ...
            copy_subtree(&forall_tmp_opnd, &forall_tmp_opnd_l, 0, 0);
            ...
            copy_subtree(&forall_tmp_opnd_l, &forall_tmp_opnd_l, 0, 0);
...
#ifdef KEY /* Bug 5091 */
	       /* A use-associated protected variable is read-only. It is
	        * slightly risky to use this boolean "constant" to mean
		* "protected" as well, since we don't want to allow such a
		* variable in a context which requires a constant, but
		* it appears that this boolean is tested only in contexts
		* which disallow writing into constants. */
	       if (readonly_protected(attr_idx)) {
	         exp_desc->constant = TRUE;
	       }
#endif /* KEY Bug 5091 */
...
# endif
   ...
   copy_subtree(&dv_opnd, &dv_opnd, 0, 0);
...
# endif
      ...
      /* Object to which we apply subscripts is allocatable. Useful to know
       * because allocatables are contiguous, so a contiguous section of an
       * allocatable results in contiguous elements, with no need to check
       * the flag in the dope vector at runtime. */
      exp_desc->allocatable_base = exp_desc_l.allocatable;
...

{
   ...
   if (attr_idx                            &&
      ...
      if (readonly_protected(attr_idx)) {
	ok = FALSE;
	PRINTMSG(line, 1702, Error, col, AT_OBJ_NAME_PTR(attr_idx));
      }
#endif /* KEY Bug 5091 */
...
#ifdef KEY /* Bug 7969 */
   /*
    * For subscript bounds check errors, it's better to cite the point of call
    * rather than the point of definition. For other problems, the point of
    * definition would be more useful. The deciding factor is that if we point
    * at the call, it's always possible to find the definition, but if we point
    * at the definition, there's no way to tell which call caused a problem. */
   copy_subtree(&stmt_func_opnd, &stmt_func_opnd, line, col);
#else /* KEY Bug 7969 */
...
#ifdef KEY /* Bug 7969 */

/* Some customers prefer warning on compile-time subscript/substring bounds
 * violation, others prefer error */
static boolean want_error() {
  static int want_error_var = -1;
  if (-1 == want_error_var) {
    char *temp = getenv("F90_BOUNDS_CHECK_ABORT");
    want_error_var = temp && 0 == strcmp(temp, "YES");
    }
  return want_error_var;
  }
#endif /* KEY Bug 7969 */

/******************************************************************************\
...

{
   ...
   boolean need_runtime_check = FALSE;
#endif /* KEY Bug 7969 */
   ...
   if (IL_FLD(IR_IDX_R(ir_idx)) == CN_Tbl_Idx                          &&
      ...
      if (compare_cn_and_value(IL_IDX(IR_IDX_R(ir_idx)), 1, Lt_Opr)) {
         ...
         find_opnd_line_and_column((opnd_type *)
                                   ...
                                   &col);
#ifdef KEY /* Bug 7969 */
	 need_runtime_check = !want_error();
         PRINTMSG(line, 1634, need_runtime_check ? Warning : Error, col);
#else /* KEY Bug 7969 */
   ...
   /* 
    * For variable, if we couldn't check at compile time, or if we printed
    * warning instead of error at compile time, we need to generate a runtime
    * check--but only if runtime checking is enabled globally, or if
    * it's enabled via directive for this particular variable.
    */
   if (need_runtime_check && base_attr &&
     ((cdir_switches.bounds || ATD_BOUNDS_CHECK(base_attr))
       && !ATD_NOBOUNDS_CHECK(base_attr))) {
     /* If we couldn't check at compile time, or if we printed warning
      * instead of error at compile time, generate runtime check. */
     boolean save_defer_stmt_expansion = defer_stmt_expansion;
     defer_stmt_expansion = FALSE;
     gen_runtime_substring(ir_idx, TRUE);
     defer_stmt_expansion = save_defer_stmt_expansion;
   }
#endif /* KEY Bug 7969 */
...

{
...
#ifdef KEY /* Bug 7969 */
   /* Now that bounds checks are warnings not errors by default, we want to
    * check at compile time regardless of -C. data_imp_do_semantics()
    * temporarily sets cdir_switches.bounds as if -C were enabled, but the
    * rest of data_stmt_semantics() does not. The following, we hope, has
    * the same effect as the code did prior to the fix for 7969: when called
    * from data_imp_do_semantics(), we goto EXIT; when called from the
    * rest of data_stmt_semantics(), we execute the body of
    * check_array_bounds() but do not emit runtime checks. */
   if (defining_stmt_func || ! needs_bounds_check(ir_idx,
      cdir_switches.bounds || !cdir_switches.in_data_stmt)) {
      goto EXIT;
   }
   ...
   /* Want to emit checks wherever possible */
   ...
   while (list_idx != NULL_IDX) {
      ...
      boolean need_runtime_check = FALSE;
      check_ub = (BD_ARRAY_CLASS(bd_idx) != Assumed_Size ||
        i != BD_RANK(bd_idx));
      if (IL_PE_SUBSCRIPT(list_idx) &&
         ...
         if (BD_ARRAY_CLASS(bd_idx) == Assumed_Size && i == BD_RANK(bd_idx)) {
            check_ub = FALSE;
         }
      }

      /* Deferred_Shape has no BD_LB_FLD(), so unsafe to test for CN_Tbl_Idx */
	 ...
	 else if (fold_relationals(IL_IDX(list_idx), BD_LB_IDX(bd_idx, i),
	   Lt_Opr)) {

	   find_opnd_line_and_column((opnd_type *)&IL_OPND(list_idx), &line,
	     &col);
	   need_runtime_check = !want_error();
	   PRINTMSG(line, 1633, need_runtime_check ? Warning : Error, col, i);
         }
	 if (check_ub) {
	   if (BD_UB_FLD(bd_idx, i) != CN_Tbl_Idx) {
	     need_runtime_check = TRUE;
	   }
	   else if (fold_relationals(IL_IDX(list_idx), BD_UB_IDX(bd_idx, i),
	     Gt_Opr)) {

	     find_opnd_line_and_column((opnd_type *)&IL_OPND(list_idx), &line,
	       &col);
	     need_runtime_check = !want_error();
	     PRINTMSG(line, 1633, need_runtime_check ? Warning : Error, col,
	       i);
	   }
	 }
      }
	...
	IL_FLD(IL_NEXT_LIST_IDX(IR_IDX_L(IL_IDX(list_idx)))) == CN_Tbl_Idx &&
	IL_FLD(IL_NEXT_LIST_IDX(IL_NEXT_LIST_IDX(
		    IR_IDX_L(IL_IDX(list_idx))))) == CN_Tbl_Idx &&
	BD_LB_FLD(bd_idx, i) == CN_Tbl_Idx &&
	BD_UB_FLD(bd_idx, i) == CN_Tbl_Idx) {

	COPY_OPND(start_opnd, IL_OPND(IR_IDX_L(IL_IDX(list_idx))));
	COPY_OPND(end_opnd,
	  IL_OPND(IL_NEXT_LIST_IDX(IR_IDX_L(IL_IDX(list_idx)))));
	COPY_OPND(inc_opnd, IL_OPND(IL_NEXT_LIST_IDX(IL_NEXT_LIST_IDX(IR_IDX_L(
	  IL_IDX(list_idx))))));

	gen_opnd(&lb_opnd, BD_LB_IDX(bd_idx, i), BD_LB_FLD(bd_idx, i), line, 
	  col);
	gen_opnd(&ub_opnd, BD_UB_IDX(bd_idx, i), BD_UB_FLD(bd_idx, i), line, 
	  col);

	gen_rbounds_condition(&cond_opnd, &start_opnd, &end_opnd, &inc_opnd,
	  &lb_opnd, check_ub ? (&ub_opnd) : 0, line, col);

# ifdef _DEBUG
         if (OPND_FLD(cond_opnd) != CN_Tbl_Idx ||
	   TYP_TYPE(CN_TYPE_IDX(OPND_IDX(cond_opnd))) != Logical) {
	   PRINTMSG(line, 626, Internal, col, "LOGICAL CN_Tbl_Idx",
	      "check_array_bounds");
         }
# endif
	 ...
	 need_runtime_check = TRUE;
      ...
      /* 
       * If we couldn't check at compile time, or if we printed warning
       * instead of error at compile time, we need to generate a runtime
       * check--but only if runtime checking is enabled globally, or if
       * it's enabled via directive for this particular variable.
       *
       * But we never want runtime bounds checking for a DATA statement
       * expression (though, for mysterious reasons, data_imp_do_semantics()
       * saves cdir_switches.bounds, sets it, and restores it.)
       */
      if (need_runtime_check &&
	(cdir_switches.in_data_stmt <= 0) &&
	((cdir_switches.bounds || ATD_BOUNDS_CHECK(base_attr))
	  && !ATD_NOBOUNDS_CHECK(base_attr))) {
	boolean save_defer_stmt_expansion = defer_stmt_expansion;
	defer_stmt_expansion = FALSE;
	gen_runtime_bounds(ir_idx, TRUE);
	defer_stmt_expansion = save_defer_stmt_expansion;
      }
...
# endif
   ...
   copy_subtree(&(exp_desc->bias_opnd), &opnd, 0, 0);
