...
EXP_PHI::Print(FILE *fp, BOOL show_preds_of_unavail) const
{
  ...
  for (i = 0; i < Opnd_count(); i++) {
	...
	fprintf(fp, "<has-real-occ>");
      ...
      fprintf(fp, "%" PRIdPTR , Opnd(i)->E_version());
  ...
  if (! Reverse_phi()) {
	...
	if (Pred(i) == NULL)
	  ...
	  case EXP_OCCURS::OCC_PHI_PRED_OCCUR:
	    fprintf(fp, "<phi_pred>%" PRIdPTR , Pred(i)->Enclosed_in_bb()->Id());
	    break;
	  case EXP_OCCURS::OCC_REAL_OCCUR:
	    fprintf(fp, "<real>%" PRIdPTR , Pred(i)->Enclosed_in_bb()->Id());
	    break;
	  default:
	    fprintf(fp, "<bad>");
	    break;
    ...
    for (i = 0; i < Opnd_count(); i++) {
      if (Succ(i) == NULL)
        ...
        case EXP_OCCURS::OCC_PHI_PRED_OCCUR:
	  fprintf(fp, "<phi_succ>%" PRIdPTR , Succ(i)->Enclosed_in_bb()->Id());
	  break;
        case EXP_OCCURS::OCC_REAL_OCCUR:
	  fprintf(fp, "<real>%" PRIdPTR , Succ(i)->Enclosed_in_bb()->Id());
	  break;
        default:
	  fprintf(fp, "<bad>");
	  break;
  ...
  if (!Identity()) {
	...
	fprintf(fp, "%" PRIdPTR , Identical_to()->E_version());
	if (Identity_injured()) {
	  fprintf(fp, "<inj>");
	}
      }
...
ETABLE::Generate_cur_expr(const BB_NODE *bb, INT opnd_num, CODEREP *newcr, BOOL fix_zero_ver)
{
  ...
  if (Tracing()) {
    fprintf(TFile, "ETABLE::Generate_cur_expr for phi opnd %d in BB%" PRIdPTR "\n",
	    opnd_num, bb->Id());
...
#if defined( KEY) // bug 4674: restore original expression in occurrence node so later
...
EXP_WORKLST::Save_use_cr(const ETABLE *etable, CODEREP * old_cr)
{
  ...
  case NEED_CVT:
	...
	old_cr->Set_sign_extension_flag();
#ifdef TARG_MIPS
      } else if (opc == OPC_I4U8CVT) {
	// Bug 14727: Note that these type really are correct for MIPS;
	// I4U8CVT corresponds to the same zero extension as U8U4LDID.
	// See bug 13308 for details.
	old_cr->Set_dtyp(MTYPE_U8);
	old_cr->Set_dsctyp(MTYPE_U4);
	old_cr->Set_sign_extension_flag();
#endif
...
ETABLE::Perform_PRE_optimization(void)
{
  ...
  Is_Trace(Tracing(),
  	   (TFile, "NEWPRE: initial worklist has %" PRIdPTR " candidates\n", _cur_e_num));
  ...
  FOR_ALL_NODE(cur_worklst, worklst_iter, Init()) {
	...
	cur_worklst->Verify();
      ...
      Is_Trace(Tracing(),
  	       (TFile, "NEWPRE: entire worklist has %" PRIdPTR " candidates\n", _cur_e_num));
...
static void Cleanup_loop_flags(BB_LOOP *first_loop, BOOL trace)
{
  if (first_loop != NULL) {
    ...
    FOR_ALL_NODE(loop, loop_iter, Init(first_loop)) {
      Is_Trace(trace, (TFile, "BB%" PRIdPTR " ", loop->Body()->Id()));
...
EXP_OCCURS::Print(FILE *fp, BOOL show_phi_preds) const
{
  ...
  if (Occ_kind() != OCC_PHI_PRED_OCCUR ||
    ...
    fprintf(fp, " Ver%02" PRIdPTR " ", E_version());
  else
    fprintf(fp, "       ");
  switch (Occ_kind()) {
  case OCC_PHI_PRED_OCCUR:
  case OCC_REAL_OCCUR:
	...
	if (Occ_kind() != OCC_REAL_OCCUR) {
	  fprintf(fp, " (BAD KIND in BB%" PRIdPTR " Sid%d)",
		  Enclosed_in_stmt()->Bb()->Id(),
		  Enclosed_in_stmt()->Stmt_id());
	}
	else {
	  fprintf(fp, " (real in BB%" PRIdPTR " Sid%d)",
		  Bb()->Id(),
    ...
    break;
  case OCC_PHI_OCCUR:
    fprintf(fp, " (%s in BB%" PRIdPTR ") ", (For_spre()? "iphi" : "phi"), Bb()->Id());
    if (T_ver_owns_coderep()) 
      fprintf(fp, " (owns coderep)");
    Exp_phi()->Print(fp, show_phi_preds);
    break;
  case OCC_COMP_OCCUR:
    fprintf(fp, " (comp in BB%" PRIdPTR " Sid%d)%s\n",
	    Enclosed_in_stmt()->Bb()->Id(),
	    Enclosed_in_stmt()->Stmt_id(),
	    (Obsolete_comparison() ? " (obsolete)" : ""));
    break;
  case OCC_EXIT_OCCUR:
    fprintf(fp, "(exit in BB%" PRIdPTR ")\n",((BB_NODE *) Enclosed_in_bb())->Id());
    break;
  default:
...
{
  if (Pre_kind() != PK_SPRE)
    fprintf(fp,"<E_NUM=%" PRIdPTR "|PREG=%" PRIdPTR , E_num(), Preg());
  else
    fprintf(fp,"<E_NUM=%" PRIdPTR , E_num());
  if (Exp() != NULL) {
    Exp()->Print_node(1, fp);
  } else {
