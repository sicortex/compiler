	       ...
	       const Sym* elf_symtab) 
{
    ...
    get_global_symtab (gtabs, IP_FILE_HDR_file_info (file_header), p_obj,
		       gsymtab); 
#if !defined(TARG_IA64) && !defined(TARG_X8664) && !defined(TARG_MIPS)    // Merge the new ELF symbol table entries with the existing ones.
    //
...
// why is this target-dependent? Probably we should replace with KEY
