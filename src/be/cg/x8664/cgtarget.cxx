...
void CGTARG_Compute_Branch_Parameters(INT32 *mispredict, INT32 *fixed, INT32 *brtaken, double *factor)
{
  ...
  *factor = 0.0;
  ...
  if (Is_Target_x86_64() )
  {
    *mispredict= 7; *fixed= 1; *brtaken= 1; *factor = 0.01;
  }
  else
  {
...
#ifdef TARG_X8664
  // Exp_Select_And_Condition can generate a compare followed by 2
  // conditional branches.  The second branch, jnp, will not have
  // a cmp_op in its BB.  Search for the cmp_op in the first BB.
  if (cmp_op == NULL) {
	...
	default: FmtAssert(FALSE, ("NYI")); 
               ...
               (WN_operator(load) == OPR_LDID &&
                (WN_desc(load) == MTYPE_U4 ||
                 WN_desc(load) == MTYPE_U8))) // bugs 14427, 14765
        rtype = WN_rtype(load);
    } else {
...
static const char* int_reg_names[4][16] = {
  // for ymm and xmm register, they also have the same alias with rax and eax. 
  // so the name int_reg_names is not so typical. 
  /* BYTE_REG: low 8-bit */
...
CGTARG_Modified_Asm_Opnd_Name(char modifier, TN* tn, char *tn_name)
{
  ...
  if (modifier == 'r') {
	...
	if( rc == ISA_REGISTER_CLASS_float){
	  ...
	  if ( TN_size(tn) == 32 ) 
        ...
        return int_reg_names[3][reg - REGISTER_MIN];
	}
  }
  else {
...
*/
TN* CGTARG_Process_Asm_m_constraint (WN* load, void** offset,
				     int clobber_count,
				     OPS *post_asm_ops, OPS* ops )
{
  ...
  if( WN_operator(load) == OPR_LDA ){
    ...
    */
    ...
    // A large ASM clobber set may cause the register allocator to run out of
    // registers.  In that case, copy the memory operand to the stack and
    // address it via the stack pointer.  Bug 14797.
    TYPE_ID mtype = WN_rtype(load);
    int size = MTYPE_byte_size(mtype);
    int alloc_set_size =
      REGISTER_SET_Size(REGISTER_CLASS_allocatable(ISA_REGISTER_CLASS_integer));

    if (MTYPE_is_integral(mtype) &&
	clobber_count > alloc_set_size - 2) {	// adjust as necessary

      FmtAssert(!(Is_Target_32bit() && size > 4),
	("CGTARG_Process_Asm_m_constraint: size too large for m32 'm' opnd"));

      const TY_IDX ty = MTYPE_To_TY(mtype);
      ST *mem_loc = Gen_Temp_Symbol(ty, "asm_m_opnd");
      Allocate_Temp_To_Memory(mem_loc);

      ST *base_sym = NULL;
      INT64 base_ofst = 0;
      Base_Symbol_And_Offset_For_Addressing(mem_loc, 0, &base_sym, &base_ofst);
      FmtAssert(base_sym == SP_Sym || base_sym == FP_Sym,
	("CGTARG_Process_Asm_m_constraint: base symbol is not at stack"));
      TN* base_tn = base_sym == SP_Sym ? SP_TN : FP_TN;
      TN* ofst_tn = Gen_Literal_TN(base_ofst, 4);
      TN *tmp_tn = Build_TN_Of_Mtype(mtype);

      TOP ld_top = size > 4 ? TOP_ld64 : TOP_ld32;
      TOP store_top = size > 4 ? TOP_store64 : TOP_store32;

      // The last OP defines the pointer TN returned by the LDID.
      OP *last_op = OPS_last(ops);
      FmtAssert(OP_results(last_op) == 1,
	("CGTARG_Process_Asm_m_constraint: unexpected last OP"));
      TN *orig_ptr_tn = OP_result(last_op, 0);

      // Copy from original memory location to stack.
      Build_OP(ld_top, tmp_tn, orig_ptr_tn, Gen_Literal_TN(0, 4), ops);
      CGTARG_Store_To_Memory(tmp_tn, mem_loc, ops);

      // Copy from stack to original memory location.
      CGTARG_Load_From_Memory(tmp_tn, mem_loc, post_asm_ops);
      Build_OP(store_top, tmp_tn, orig_ptr_tn, Gen_Literal_TN(0, 4),
	       post_asm_ops);

      // Change the ASM operand.
      *offset = (void*) Gen_Literal_TN(TN_value(ofst_tn), 4); 
      asm_opnd = base_tn;
    }
  } else {
