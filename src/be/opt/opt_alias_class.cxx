...
ALIAS_CLASSIFICATION::New_base_id(const ST *st, TY_IDX ty)
{
  ...
  if (Tracing()) {
    fprintf(TFile, "Allocating base ID %" PRIdPTR "\n", id);
  }
  ...
  if (Tracing()) {
    fprintf(TFile, "Base ID %3" PRIdPTR " is ", id);
  }
			      ...
			      const TY_IDX        ty)
{
  ...
  if (ST_sclass(base_st) == SCLASS_REG) {
    ...
    if (base_id == (IDTYPE) 0) {
      ...
      if (Tracing()) {
	fprintf(TFile, "base_ofst is %" SCNd64 "\n", base_ofst);
      }
...
ALIAS_CLASS_REP::Print(FILE *fp, ALIAS_CLASS_REP *global_class) const
{
  ...
  fprintf(fp, " -->");
  if (Is_pointer_class()) {
    fprintf(fp, " class %" PRIdPTR " ", Class_pointed_to()->Id());
    if (global_class != NULL &&
...
{
  if (_kind == ACM_BASE) {
    fprintf(fp, "base ID %" PRIdPTR "", Base_id());
  }
					     ...
					     BOOL expr_must_point)
{
  ...
  if (OPCODE_is_leaf(opc)) {
	...
	if ((initv = ST_has_initv(lhs_st)) &&
	  ...
	  if (Tracing()) {
	    fprintf(TFile, "ALIAS_CLASSIFICATION: process initialization lhs=%" PRIdPTR " rhs=%" PRIdPTR "\n",
...
*/
  ...
  // information available to the callee.
    ...
    AC_PTR_OBJ_PAIR t = Classify_deref_of_expr(
		      opr != OPR_ILOADX ? WN_kid0(expr) : WN_kid1(expr), TRUE);
...
ALIAS_CLASSIFICATION::Classify_lhs_of_store(WN *const stmt)
{
  ...
  if (OPERATOR_is_scalar_store (opr)) {
    ...
    AC_PTR_OBJ_PAIR lhs_ptr_class =
      Classify_deref_of_expr(opr != OPR_ISTOREX ? WN_kid1(stmt) : WN_kid2(stmt),
      			     TRUE);
	...
	if (pt->Alias_class() == OPTIMISTIC_AC_ID) {
	  ...
	  if ((pt->Alias_class() != class_id) &&
	    ...
	    DevWarn("ALIAS_CLASSIFICATION: Inconsistent alias class "
		    "for base ID %" PRIdPTR ", aux ID %" PRIdPTR "", WN_base_id(wn), WN_aux(wn));
