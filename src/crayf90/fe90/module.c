...
#ifdef KEY /* Bug 14481 */
      if (ATI_FIRST_SPECIFIC_IDX(at_idx) != NULL_IDX) {
	ATI_FIRST_SPECIFIC_IDX(at_idx) =
	  ML_SN_IDX(ATI_FIRST_SPECIFIC_IDX(at_idx));
      }
#else /* KEY Bug 14481 */
...
#if defined(__LP64__) /* gcc and most all modern compilers add this with -m64 which is really what we're testing for */
   MD_POOLS_64_BITS		= 1;
#else
   MD_POOLS_64_BITS		= 0;
#endif
...
EXIT:
   ...
   while (al_idx != NULL_IDX) {
      ...
      if (host_attr_idx != NULL_IDX &&
	   ...
	   ATI_FIRST_SPECIFIC_IDX(attr_idx) = ATI_FIRST_SPECIFIC_IDX(host_attr_idx);
	 }
	 else {
#endif /* KEY Bug 14481 */
...
FOUND:
   ...
   /* Because name pool and constant pool use long_type, the structure of
    * a module file generated by a compiler built with -m64 (targeting either
    * -m32 or -m64) differs from that of a compiler built with -m32 (targeting
    * either -m32 or -m64.) */
   if (
#if defined(_HOST64)
     !
#endif // defined(_HOST64)
     MD_POOLS_64_BITS
   ) {
...
# endif
	 ...
	 if (ATT_PARENT_TYPE(attr_idx) != NULL_IDX)
      ...
      case Interface:
         ...
         ML_AT_SEARCHED(attr_idx)	   = TRUE;
#ifdef KEY /* Bug 14481 */
	 if (ATI_FIRST_SPECIFIC_IDX(attr_idx) != NULL_IDX) {
